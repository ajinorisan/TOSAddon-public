...
4人以下を押した時の確認を削除、レイドレコードのサイズ変更及び場所移動を可能に、パーティーバフの表示を選択出来る様に、パーフェクト有効化無効化をシステムチャットに非表示に、チャンネル表示のバグ修正(日本版のみ)、レイド時に右上のミニボタンを非表示に、街に移動時にマーケットボタンなどを自動で表示、死んだ時のダイアログを動かせるように、CC時にペット召喚画面を表示、各種ダイアログ制御(倉庫番、住居クポル、各種レイド)、オートキャスティング設定をキャラ毎に設定
