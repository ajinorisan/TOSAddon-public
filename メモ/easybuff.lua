...
function EASYBUFF_SQUIRE_BUFF_EQUIP_CTRL(frame)
    ...
    CHAT_SYSTEM("EASYBUFF_SQUIRE_BUFF_EQUIP_CTRL called")
    CHAT_SYSTEM("useHook value: " .. g.settings.useHook)
    --frame:RunUpdateScript("EASYBUFF_WAIT_AND_RUN_TEST")
    EASYBUFF_SQUIRE_BUFF_TEST(frame)
end

function EASYBUFF_SQUIRE_BUFF_TEST(frame)
    CHAT_SYSTEM("TEST")
    local checkall = GET_CHILD_RECURSIVELY(frame, 'checkall')
    
    -- 個々のアイテムのチェックボックスの状態を変更
    ...
    -- チェックボックスの状態を変更した後に、checkall チェックボックスをチェックする
    CHAT_SYSTEM("Checkall before: " .. checkall:IsChecked())
    checkall:SetCheck(1)
    CHAT_SYSTEM("Checkall after: " .. checkall:IsChecked())

    -- 全てのチェックボックスがチェックされた後に、SQUIRE_BUFF_EQUIP_SELECT_ALL を呼び出す
