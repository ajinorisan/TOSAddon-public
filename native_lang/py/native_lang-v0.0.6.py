...
# Version 0.0.6 (Final, full_snake_case, simplified)
...
# ACTIVE_ID = str(76561198795373702) # // テスト用の固定IDはコメントアウト
...
if __name__ == "__main__":
    ...
    try:
        ...
        show_error_and_exit("【緊急事態】捕捉不能な致命的エラー", f"プログラムの予期せぬ場所でエラーが発生しました。\n\nエラー詳細: {e}")
    finally:
        if manager:
            manager.stop()
        terminate_self()
...
if __name__ == "__main__":
    ...
    try:
        ...
        show_error_and_exit("【緊急事態】捕捉不能な致命的エラー", f"プログラムの、予期せぬ場所で、エラーが発生しました。\n\nエラー詳細: {e}")
        if manager:
            manager.stop()
        terminate_self()
        ...
        if manager and manager._is_stopping:
