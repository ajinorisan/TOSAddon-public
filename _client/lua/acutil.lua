...
function acutil.slashSet(set)
    if (not set.base) then
        return ui.SysMsg('[acutil.slashSet] missing "base" string.');
    end
    if (set.title) then
    ...
    local executeSetCmd = function(fn, params)
        ...
        if (p1) then
            ...
            if (p2 and p3) then
                return ui.MsgBox(msg, p2, p3);
            end
            if (p2) then
                return ui.MsgBox(msg, p2);
            end
...
function acutil.addSysIcon(name, icon, tooltip, functionString)
    if ACUTIL_sysmenuAddons == nil then
        ACUTIL_sysmenuAddons = {};
    end
    if ACUTIL_sysmenuAddons[name] == nil then
        ACUTIL_sysmenuAddons[name] = {};
    end
...
function SYSMENU_CREATE_VARICON_HOOKED(frame, status, ctrlName, frameName, imageName, startX, offsetX, hotkeyName)
    local margin = startX;
    margin =
        _G["SYSMENU_CREATE_VARICON_OLD"](frame, status, ctrlName, frameName, imageName, startX, offsetX, hotkeyName);
