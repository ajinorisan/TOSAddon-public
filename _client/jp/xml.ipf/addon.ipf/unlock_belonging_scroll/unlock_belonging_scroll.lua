function UNLOCK_BELONGING_SCROLL_ON_INIT(addon, frame)
end

function UNLOCK_BELONGING_SCROLL_TARGET_ITEM_SLOT(slot, invItem, scrollClsID)
...
function UNLOCK_BELONGING_SCROLL_EXEC_ASK_AGAIN(frame, btn)
	...
	local yesscp = 'UNLOCK_BELONGING_SCROLL_EXEC'
...
function UNLOCK_BELONGING_SCROLL_RESULT(isSuccess)
	local frame = ui.GetFrame("unlock_belonging_scroll");
	if isSuccess == 1 then
		local animpic_bg = GET_CHILD_RECURSIVELY(frame, "animpic_bg");
		animpic_bg:ShowWindow(1);
		animpic_bg:ForcePlayAnimation();
		ReserveScript("UNLOCK_BELONGING_SCROLL_CHANGE_BUTTON()", 0.3);
	else
		UNLOCK_BELONGING_SCROLL_RESULT_UPDATE(frame, 0);
	end
	
	UNLOCK_BELONGING_SCROLL_LOCK_ITEM("None");
	
	local slot = GET_CHILD(frame, "slot");
	local icon = slot:GetIcon();
	icon:SetTooltipType("None");
	icon:SetTooltipArg("", 0, "");
	ReserveScript("UNLOCK_BELONGING_SCROLL_CHANGE_TOOLTIP()", 0.3);
end

function UNLOCK_BELONGING_SCROLL_CHANGE_TOOLTIP()
	local frame = ui.GetFrame("unlock_belonging_scroll");
	local slot = GET_CHILD(frame, "slot");
	local icon = slot:GetIcon();
	local invItem = GET_SLOT_ITEM(slot);
...
function UNLOCK_BELONGING_SCROLL_CHANGE_BUTTON()
	local frame = ui.GetFrame("unlock_belonging_scroll");
...
function UNLOCK_BELONGING_SCROLL_RESULT_UPDATE(frame, isSuccess)
	local slot = GET_CHILD(frame, "slot");
	...
	frame:StopUpdateScript("TIMEWAIT_STOP_unlock_belonging_scroll");
	frame:RunUpdateScript("TIMEWAIT_STOP_unlock_belonging_scroll", timesecond);
end

function TIMEWAIT_STOP_unlock_belonging_scroll()
	local frame = ui.GetFrame("unlock_belonging_scroll");
	local slot_temp = GET_CHILD(frame, "slot_temp");
...
function UNLOCK_BELONGING_SCROLL_BG_ANIM_TICK(ctrl, str, tick)
	if tick == 10 then
		local frame = ctrl:GetTopParentFrame();
		local animpic_slot = GET_CHILD_RECURSIVELY(frame, "animpic_slot");
		animpic_slot:ForcePlayAnimation();	
		ReserveScript("UNLOCK_BELONGING_SCROLL_EFFECT()", 0.3);
	end
end

function UNLOCK_BELONGING_SCROLL_EFFECT()
	local frame = ui.GetFrame("unlock_belonging_scroll");
	UNLOCK_BELONGING_SCROLL_RESULT_UPDATE(frame, 1);	
end

function UNLOCK_BELONGING_SCROLL_EXEC()
	local frame = ui.GetFrame("unlock_belonging_scroll");		
	...
	item.DialogTransaction("ITEM_UNLOCK_BELONGING_SCROLL", resultlist);
	
	imcSound.PlaySoundEvent(frame:GetUserConfig("TRANS_CAST"));
end

function UNLOCK_BELONGING_SCROLL_CANCEL()
	UNLOCK_BELONGING_SCROLL_LOCK_ITEM("None");
end

function UNLOCK_BELONGING_SCROLL_CLOSE()
	local frame = ui.GetFrame("unlock_belonging_scroll");
	...
	UNLOCK_BELONGING_SCROLL_LOCK_ITEM("None")
	UNLOCK_BELONGING_SCROLL_UI_RESET();
	UNLOCK_BELONGING_SCROLL_CANCEL();
...
function UNLOCK_BELONGING_SCROLL_LOCK_ITEM(guid)
	...
	invframe:SetUserValue("ITEM_GUID_IN_unlock_belonging_scroll", guid);
	INVENTORY_ON_MSG(invframe, "UPDATE_ITEM_unlock_belonging_scroll", lockItemGuid);
end

function UNLOCK_BELONGING_SCROLL_UI_INIT()
	local frame = ui.GetFrame("unlock_belonging_scroll");
	local scrollGuid = frame:GetUserValue("ScrollGuid")	
	local scrollInvItem = session.GetInvItemByGuid(scrollGuid);
	if scrollInvItem == nil then
	...
	text_title:SetTextByKey('value', dic.getTranslatedStr(scrollObj.Name))
...
function UNLOCK_BELONGING_SCROLL_UI_RESET()
	local frame = ui.GetFrame("unlock_belonging_scroll");
...
function UNLOCK_BELONGING_SCROLL_INV_RBTN(itemObj, slot)	
	local icon = slot:GetIcon();
	local iconInfo = icon:GetInfo();
	local invItem = GET_PC_ITEM_BY_GUID(iconInfo:GetIESID());

	local invframe = ui.GetFrame("inventory");
	UNLOCK_BELONGING_SCROLL_SET_TARGET_ITEM(invframe, invItem)
end

function UNLOCK_BELONGING_SCROLL_ITEM_DROP(parent, ctrl)
...
function UNLOCK_BELONGING_SCROLL_SET_TARGET_ITEM(invframe, invItem)
	local frame = ui.GetFrame("unlock_belonging_scroll");
	...
	local ret, msg = item_goddess_craft.is_able_to_unlock_scroll(scrollObj, itemObj)
	if ret == false then
		ui.SysMsg(ClMsg(msg))
		return
	end
	...
	UNLOCK_BELONGING_SCROLL_CANCEL();
	UNLOCK_BELONGING_SCROLL_TARGET_ITEM_SLOT(slot, invItem, scrollObj.ClassID);
	UNLOCK_BELONGING_SCROLL_LOCK_ITEM(invItem:GetIESID())

	frame:SetUserValue("EnableTranscendButton", 1);	
	frame:OpenFrame(1);
end

function UNLOCK_BELONGING_SCROLL_CHECK_TARGET_ITEM(slot)-- _CHECK_MORU_TARGET_ITEM
	local frame = ui.GetFrame("unlock_belonging_scroll");	
...
function UNLOCK_BELONGING_SCROLL_SELECT_TARGET_ITEM(scrollItem)	
	...
	UNLOCK_BELONGING_SCROLL_CANCEL();	
	UNLOCK_BELONGING_SCROLL_UI_INIT();
	UNLOCK_BELONGING_SCROLL_UI_RESET();
	frame:ShowWindow(1);
	...
	ui.SetEscapeScp("UNLOCK_BELONGING_SCROLL_CANCEL()");
		
	local tab = gbox:GetChild("inventype_Tab");	
	tolua.cast(tab, "ui::CTabControl");
	tab:SelectTab(1);

	SET_SLOT_APPLY_FUNC(invframe, "UNLOCK_BELONGING_SCROLL_CHECK_TARGET_ITEM", nil, "Equip");
	INVENTORY_SET_CUSTOM_RBTNDOWN("UNLOCK_BELONGING_SCROLL_INV_RBTN");
end
