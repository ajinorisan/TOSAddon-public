...
--    value = value * JAEDDURY_MON_DEF_RATE;      -- JAEDDURY
    ...
    if IsServerSection(self) == 1 then
        zone_name = GetZoneName(self);
    else
        zone_name = GetZoneName();
    end

    if zone_name ~= nil and zone_name ~= "None" then
        local prefix = 'field_monster_status_' .. zone_name
        if IsServerSection(self) == 1 then
        if IsIndun(self) == 1 then
            prefix = prefix .. '_indun'
        end
        else
            if session.world.IsDungeon() == true then
                prefix = prefix .. '_indun'
            end    
        end
        if GetExProp(self, 'IsChallengeModeMon') == 0 then
            local spec_cls = GetClass(prefix, self.ClassName)
            if spec_cls ~= nil then                                
                value = revise_mon_def(self, TryGetProp(spec_cls, 'DEF', 0))
            else                
                value = revise_mon_def(self, math.floor(value))
            end
        end
    end
...
function SCR_Get_MON_MDEF(self)
    ...
    local byItem = 0;
    local className = TryGetProp(self, "ClassName");
    if className ~= nil and (className == "pcskill_skullsoldier" or className == "pcskill_skullarcher" or className == "pcskill_skullwizard" or className == "Vibora_Spiritsoldier" or className == "Vibora_Spiritarcher" or className == "Vibora_Spiritwizard") then
        byItem = SCR_MON_OWNERITEM_ARMOR_CALC(self);
    else
...
--    value = value * JAEDDURY_MON_DEF_RATE;      -- JAEDDURY
    ...
    if GetExProp(self, 'IsChallengeModeMon') == 0 then
        local spec_cls = GetClass(prefix, self.ClassName)
        if spec_cls ~= nil then
            value =  revise_mon_def(self, TryGetProp(spec_cls, 'MDEF', 0));
        else
            value = revise_mon_def(self, math.floor(value));
        end
    end
