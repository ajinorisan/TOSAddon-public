...
function NEWBIE_MAKE_TREE(frame)
	...
	if GetServerNation() == "PAPAYA" then
		clsList, cnt = GetClassList('TPitem_User_New_PAPAYA');	
	end
...
function UPDATE_NEWBIE_BASKET_MONEY(frame) 
	...
	for i = 0, slotCount - 1 do
		...
		if slotIcon ~= nil then
			...
			if GetServerNation() == "PAPAYA" then
				alreadyItem = GetClass("TPitem_User_New_PAPAYA",classname)
			end
...
function TPSHOP_ITEM_TO_NEWBIE_BUY_BASKET_PREPROCESSOR(parent, control, tpitemname, tpitem_clsID)	
	...
	if GetServerNation() == "PAPAYA" then
		obj = GetClassByType("TPitem_User_New_PAPAYA", tpitem_clsID)
	end
	...
	elseif limit == "DAILY" then
		...
		if curBuyCount >= obj.AccountLimitDailyCount then
			...
			ui.MsgBox(ScpArgMsg("SelectPurchaseRestrictedItemByDaily", "Value", obj.AccountLimitDailyCount, "Value2", obj.AccountLimitDailyCount - curBuyCount), string.format("TPSHOP_ITEM_TO_NEWBIE_BUY_BASKET('%s', %d)", tpitemname, classid), "None");
		end
	elseif limit == 'CUSTOM' then
...
function TPSHOP_ITEM_TO_NEWBIE_BUY_BASKET(tpitemname, classid)		
	...
	if GetServerNation() == "PAPAYA" then
		tpitem = GetClass("TPitem_User_New_PAPAYA", tpitemname)
	end
