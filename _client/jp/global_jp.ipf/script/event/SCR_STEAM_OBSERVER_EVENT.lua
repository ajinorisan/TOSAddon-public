...
--    if aObj.HANGEUL_EVENT_COUNT5 > 10 or aObj.DAYCHECK_EVENT_REWARD_DATE == year..'/'..yday then
--        return
--    end
...
--    local aObj = GetAccountObj(pc)
--    local sObj = GetSessionObject(pc, 'ssn_klapeda')
--    local EventDay = 'DAYCHECK_EVENT_REWARD_DATE'
--    local EventCount = 'HANGEUL_EVENT_COUNT5'
--    local questTable = { 
...
-- --    if aObj.HANGEUL_EVENT_BOX5 == 1 and aObj.DAYCHECK_EVENT_REWARD_DATE ~= year..'/'..yday then
-- --        local tx = TxBegin(pc)
-- --        TxSetIESProp(tx, aObj, 'HANGEUL_EVENT_BOX5', 0);
-- --    	local ret = TxCommit(tx)
-- --    end
   
--    if aObj.HANGEUL_EVENT_COUNT5 >= 10 then
--        ShowOkDlg(pc, 'EV_TREASURE_DESC_05', 1)
--        return
--    end
   
--    if aObj.DAYCHECK_EVENT_REWARD_DATE == year..'/'..yday then
...
--    if aObj.HANGEUL_EVENT_BOX5 == 1 and result ~= 0 then
--         if result == 1 then
--             ShowOkDlg(pc, 'EV_OBSERVER_DESC', 1)
--             return
--         end
        
--         if aObj.HANGEUL_EVENT_BOX5 == 1 and result ~= 2 then
--             ShowOkDlg(pc, 'EV_OBSERVER_DESC', 1)
--             return
--         end
...
--             for i = 1, table.getn(questTable) do
--                 if sObj[questTable[i]] == 1 then
--                     TxSetIESProp(tx, sObj, questTable[i], 300);
--                 end
--             end
--             local ret = TxCommit(tx)
...
--             TxSetIESProp(tx, aObj, 'HANGEUL_EVENT_BOX5', 1);
--             local ret = TxCommit(tx)
--         end
--    end
-- end