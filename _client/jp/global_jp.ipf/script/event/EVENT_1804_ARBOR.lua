...
function EVENT_1804_ARBOR_NOW_TIME()
    ...
    if nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 6, 27) then
        index = 1
    elseif nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 7, 4) then
        index = 1
    elseif nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 7, 11) then
        index = 2
    elseif nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 7, 18) then
        index = 3
    elseif nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 7, 25) then
        index = 4
    elseif nowbasicyday < SCR_DATE_TO_YDAY_BASIC_2000(2018, 8, 1) then
        index = 5
    elseif nowbasicyday <= SCR_DATE_TO_YDAY_BASIC_2000(2018, 8, 8) then
        index = 6
    else
        index = 0
    end
...
--    print('DDDDDDDDDDDDDDDDDDDDDDD',zoneObj.EVENT_1804_ARBOR_TREE_INPUT_COUNT)
    ...
    elseif select == 3 then
        if timeIndex >= 3 and (aObj.EVENT_1804_ARBOR_WEEKREWARD3 == 1 or aObj.EVENT_1804_ARBOR_WEEKREWARD4 == 1 or aObj.EVENT_1804_ARBOR_WEEKREWARD5 == 1 or aObj.EVENT_1804_ARBOR_WEEKREWARD6 == 1) then
            ...
            if select2 == 1 then
                local tx = TxBegin(pc)
                TxGiveItem(tx, 'costume_1709_NewField_m', 1, 'EVENT_1804_ARBOR_TREE')
                TxGiveItem(tx, 'costume_1709_NewField_f', 1, 'EVENT_1804_ARBOR_TREE')
    ...
    if GetZoneName(self) == 'c_Klaipe' then
        ...
        if npcIndex > 0 then
            ...
            if timeIndex >= 3 and timeIndex <= 4 and sObj.EVENT_1804_ARBOR_BALLOON1_DATE == 'None' then
                ...
                RunScript('GIVE_TAKE_SOBJ_ACHIEVE_TX', self, 'E_Balloon_7/1/E_Balloon_8/1/E_Balloon_9/1',nil,nil,nil,'EVENT_1804_ARBOR_BALLOON1','ssn_klapeda/EVENT_1804_ARBOR_BALLOON1_DATE/'..nowday)
            elseif timeIndex >= 5 and timeIndex <= 6 and sObj.EVENT_1804_ARBOR_BALLOON2_DATE == 'None'  then
                ShowOkDlg(self, npcDlg[npcIndex], 1)
                RunScript('GIVE_TAKE_SOBJ_ACHIEVE_TX', self, 'E_Balloon_10/1',nil,nil,nil,'EVENT_1804_ARBOR_BALLOON2','ssn_klapeda/EVENT_1804_ARBOR_BALLOON2_DATE/'..nowday)
            end
        end
    end
end
...
function SCR_USE_EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_1_2(pc)
	...
	if ret == 'SUCCESS' then
	    SCR_SEND_NOTIFY_REWARD(pc, GetClassString('Item','EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_1_2','ClassName'), itemList[targetIndex][1] .. ';' .. itemList[targetIndex][2])
	end
end
...
function SCR_USE_EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_3_4(pc)
	...
	if ret == 'SUCCESS' then
	    SCR_SEND_NOTIFY_REWARD(pc, GetClassString('Item','EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_3_4','ClassName'), itemList[targetIndex][1] .. ';' .. itemList[targetIndex][2])
	end
end
...
function SCR_USE_EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_5_6(pc)
	...
	if ret == 'SUCCESS' then
	    SCR_SEND_NOTIFY_REWARD(pc, GetClassString('Item','EVENT_1804_ARBOR_GROW_CRYSTAL_CUBE_5_6','ClassName'), itemList[targetIndex][1] .. ';' .. itemList[targetIndex][2])
	end
end
