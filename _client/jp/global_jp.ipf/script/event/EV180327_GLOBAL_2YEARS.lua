function SCR_USE_EVENT_STEAM_2YEARS_GUIDE(self, argstring, arg1, arg2)
    ...
    if aObj.STEAM_2YEARS_MASTER_YDAY == yday and aObj.STEAM_2YEARS_MASTER_YDAY_COUNT >= 3 then
...
function SCR_STEAM_EVENT_2YEARS_DIALOG(self, pc)
    ...
    --if aObj.STEAM_2YEARS_MASTER_DLC_CHECK == 1 then
        if aObj.STEAM_2YEARS_MASTER_COSTUME_CHECK == 0 then
            local tx = TxBegin(pc)
            TxGiveItem(tx, 'costume_Com_30', 1, "EVENT_STEAM_2YEARS");
            TxGiveItem(tx, 'costume_Com_31', 1, "EVENT_STEAM_2YEARS");
            ...
            TxGiveItem(tx, 'Hat_628035', 1, 'DLC_BOX_2YEARS');
            TxGiveItem(tx, 'Beauty_DyeCoupon_100', 1, 'DLC_BOX_2YEARS');
        ...
        if sObj.STEAM_2YEARS_MASTER_LIST_CHECK == 0 then
            ...
            SendAddOnMsg(pc, "NOTICE_Dm_scroll", ScpArgMsg("GIMMICK_DEFENSE_ERROR1"), 10)
        end
    --else
...
function SCR_STEAM_EVENT_2YEARS_MASTER_ALL(self, pc)
    ...
    if aObj.STEAM_2YEARS_MASTER_YDAY ~= yday then --오늘이 아니면 2회 카운트 초기화 --
        local tx = TxBegin(pc)
        TxSetIESProp(tx, aObj, 'STEAM_2YEARS_MASTER_YDAY_COUNT', 0); -- 2회 카운트 초기화 --
        TxSetIESProp(tx, aObj, 'STEAM_2YEARS_MASTER_YDAY', yday); 
        local ret = TxCommit(tx)
    end

    if aObj.STEAM_2YEARS_MASTER_YDAY_COUNT >= 3 then -- 2회 카운트 이상이면 리턴 --
        ShowOkDlg(pc, 'NPC_EVENT_2YEARS_DLG1', 1) 
        return
    else
        local tx = TxBegin(pc)
        TxSetIESProp(tx, aObj, 'STEAM_2YEARS_MASTER_COUNT', aObj.STEAM_2YEARS_MASTER_COUNT + 1); 
        TxSetIESProp(tx, aObj, 'STEAM_2YEARS_MASTER_YDAY_COUNT', aObj.STEAM_2YEARS_MASTER_YDAY_COUNT + 1); 
        --TxSetIESProp(tx, aObj, 'STEAM_2YEARS_MASTER_YDAY', yday); 
