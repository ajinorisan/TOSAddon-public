-- toolskill_input.lua
function SKL_KEY_DYNAMIC_CASTING(actor, obj, dik, 
	movable, rangeChargeTime, maxChargeTime, autoShot, rotateAble, 
	loopingCharge, gotoSkillUse, execByKeyDown, upAbleSec, useDynamicLevel, 
	...
	if buffName ~= nil and type(buffName) == 'string' and buffName ~= 'None' then
		...
		if buffName == 'ITEM_BUFF_VIBORA_THSWORD_PUNISH' or buffName == 'Bazooka_Buff' then
			if buff == nil then
				return 0, 1
			end
...
function SKL_KEY_GROUND_EVENT(actor, obj, dik, chargeTime, autoShot, shotCasting, lookTargetPos, selRange, upAbleSec, useDynamicLevel, isVisivle, isFullCharge, effectName, scale, nodeName, lifeTime, shockWave, shockPower, shockTime, shockFreq, shockAngle, onlyMouseMode, quickCast, hitCancel, buffName, abilName, exprop_name, exprop_value)
	if exprop_name == nil then
		exprop_name = "None";
	end
	if exprop_name ~= "None" then
		local myPC = GetMyPCObject();
		if myPC ~= nil then
			local value = GetExProp(myPC,"LamaFistCnt");
			if value ~= exprop_value then
				return 0, 1;
			end
		end
	end
...
function SKL_PARTY_TARGET_BY_KEY(actor, obj, dik, showHPGauge)
	...
	if skill_name == 'Arquebusier_DesperateDefense' or skill_name == 'Common_Arquebusier_DesperateDefense' then
	...
	if skill_name == 'Cleric_Heal' and IS_ANCIENT_HEAL_ENABLE() == 'YES' and ancientCnt > 0 then
...
function SKL_SELECT_BUFF_BY_KEY(actor, obj, dik, abilName)
	...
	local skillCls = GetClassByType("Skill",obj.type)
	local buffList = StringSplit(skillCls.SelfBuff,';')
	local upID, leftID, downID, rightID = unpack(buffList);
...
function SKL_KEY_DYNAMIC_CASTING_EXPROP_CHECK(actor, obj, dik, movable, rangeChargeTime, maxChargeTime, autoShot, rotateAble, loopingCharge, gotoSkillUse, execByKeyDown, upAbleSec, useDynamicLevel, isVisivle, isFullCharge, effectName, scale, nodeName, lifeTime, shockwave, intensity, time, frequency, angle, quickCast, hitCancel, buffName, abilName, exprop_name, exprop_value)
	if exprop_name == nil then exprop_name = "None"; end
	if exprop_name ~= "None" then
		local my_pc = GetMyPCObject();
        if my_pc ~= nil then
			local value = GetExProp(my_pc, exprop_name);
			if value ~= exprop_value then
				return 0, 1;
			end
		end
	...
	if isVisivle == nil then isVisivle = 1; end
	if isFullCharge == nil then isFullCharge = 0; end
	if effectName == nil then effectName = "None"; end
	if scale == nil then scale = 1.0; end
	if nodeName == nil then nodeName = "None"; end
	if lifeTime == nil then lifeTime = 0; end
	if shockwave == nil then shockwave = 0; end
	if intensity == nil then intensity = 0; end
	if time == nil then time = 0; end	
	if frequency == nil then frequency = 0; end 
	if angle == nil then angle = 0; end
	if quickCast == nil then quickCast = 1; end

	if quickCast ~= nil and quickCast == false then quickCast = 0;
	elseif quickCast ~= nil and quickCast == true then quickCast = 1; end
	...
	local ret = geSkillControl.DynamicCastingSkill(actor, obj.type, dik, movable, rotateAble, rangeChargeTime, maxChargeTime, autoShot, loopingCharge, gotoSkillUse, execByKeyDown, upAbleSec, isVisivle, useDynamicLevel, isFullCharge, effectName, nodeName, lifeTime, scale,1,1,1, shockwave, intensity, time, frequency, angle, quickCast, useMouseDir);

	if nil ~= hitCancel and hitCancel == 1 then
		actor:SetHitCancelCast(false)
	end

	if ret ~= nil and ret == 99 then
		return 1, 1;
	end
