...
function skill_notice_setting(frame, ctrl, str, num)
    ...
    local index = 0
    local y = 50
    for key, table in pairs(g.settings[g.cid].bufflist) do

        local buffName = newframe:CreateOrGetControl("richtext", "buffName" .. index, 10, y - 20, 200, 20)
        AUTO_CAST(buffName)
        buffName:SetText("{ol}" .. table.text or "")
        ...
        effectedit:SetText("{ol}" .. table.size or "")
        ...
        local buff = GetClassByType("Buff", table.id);
        ...
        icon:SetTooltipArg(buff.Name, "Buff", table.id, 0);
...
function skill_notice_frame_init(frame)
    ...
    local setting = frame:CreateOrGetControl("button", "setting", 160, 5, 20, 20)
    AUTO_CAST(setting)
    ...
    if not next(g.settings[g.cid].bufflist) then
        return
    end

    local buffgb = frame:CreateOrGetControl('groupbox', "buffgb", 0, 20, 0, 0);
    ...
    local colortone = ""
    local max = 0
    for index, buffData in pairs(g.settings[g.cid].bufflist) do
...
function skill_notice_buff_add(frame, msg, str, num)
    local frame = ui.GetFrame("skill_notice")
    local myHandle = session.GetMyHandle()
    local actor = world.GetActor(myHandle)
    local bufftype = num

    for index, buffData in ipairs(g.settings[g.cid].bufflist) do
