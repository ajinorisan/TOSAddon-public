...
function vakarine_equip_unequip_animas(frame)
    ...
    if g.get_map_type() == "City" and g.neck then
        ...
        frame:RunUpdateScript("vakarine_equip_neck_equip", 2.0)
    end
end
...
function vakarine_equip_frame_init(frame, msg, str, num)
    ...
    local vaka_pic = frame:CreateOrGetControl("picture", "vaka_pic", 0, 0, 35, 35);
    AUTO_CAST(vaka_pic)
...
function vakarine_equip_config_frame_open()
    ...
    for i, equip_name in ipairs(equips_tbl) do
        ...
        local check_box = config_gb:CreateOrGetControl('checkbox', "check_box" .. i, 20, i * 30 - 30, 30, 30);
        AUTO_CAST(check_box)
...
function vakarine_equip_GAME_START(frame, msg, str, num)
    ...
    if g.enter_field_boss == true then
        g.enter_field_boss = false
        return
    end
