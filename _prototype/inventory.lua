...
function UPDATE_INVENTORY_SLOT(slot, invItem, itemCls)
    ...
    if ui.GetFrame('shop'):IsVisible() == 1 or ui.GetFrame('exchange'):IsVisible() == 1 or
        ui.GetFrame("oblation_sell"):IsVisible() == 1 then
...
function INIT_INVEN_SLOT(slot)
    ...
    if shopframe:IsVisible() == 1 or exchangeframe:IsVisible() == 1 or companionshop:IsVisible() == 1 or
        oblationSell:IsVisible() == 1 then
    ...
    for typeNo = 1, #g_invenTypeStrList do
        ...
        for i = 1, #SLOTSET_NAMELIST do
            ...
            for j = 0, slotSet:GetChildCount() - 1 do
                ...
                if slotItem ~= nil and
                    (oblationSell:IsVisible() == 1 or shopframe:IsVisible() == 1 or exchangeframe:IsVisible() == 1) then
...
function TRY_TO_USE_WARP_ITEM(invitem, itemobj)
    ...
    if IsBuffApplied(pc, 'Event_Penalty') == 'YES' or IsBuffApplied(pc, 'PVP_MINE_BUFF1') == 'YES' or
        IsBuffApplied(pc, 'PVP_MINE_BUFF2') == 'YES' then
        ui.SysMsg(ScpArgMsg("CannotUseThieInThisMap"));
        return 0;
    end
...
function CHECK_EXEC_DELETE_ITEMDROP(count, className)
    ...
    if item_grade >= 3 then
        clmsg = ScpArgMsg('HighItemGradeReallyDestroy{ITEM}{COUNT}', 'ITEM', s_dropDeleteItemName, 'COUNT',
                          s_dropDeleteItemCount);
    end
