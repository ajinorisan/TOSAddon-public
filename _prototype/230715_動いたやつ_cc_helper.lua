...
function CC_HELPER_CARD_SLOT_DROP(frame, slot, argStr, argNum)
    CHAT_SYSTEM(tostring(slot))
...
function cc_helper_card_equip()
    ...
    if goditem ~= nil then
        ReserveScript(string.format("cc_helper_legcard_equip(%d, '%s')", godcardslot, g.godiesid), 0.5)
    end
...
--    local ark = session.GetInvItemByGuid(tonumber(g.arkiesid));
  --  
    ...
    elseif ark ~= nil then
        item.PutItemToWarehouse(IT_ACCOUNT_WAREHOUSE, ark, 1, nil, nil)
        ReserveScript("cc_helper_inv_to_warehouse()", 0.5)
        return
