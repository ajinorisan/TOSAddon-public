...
function sub_map_CREATE_PM_PICTURE(frame, pcInfo, type, mapprop)
    ...
    local gbox = GET_CHILD(frame, "gbox") -- gbox を先に取得

    -- ★★★ 修正ポイント ★★★
    -- PM_ でも GM_ でも、とにかくこのAIDのアイコンが既に gbox にないかチェック！
    if gbox:GetChild("PM_" .. aid) ~= nil or gbox:GetChild("GM_" .. aid) ~= nil then
        -- どっちかのアイコンが既にあるなら、何もしないで帰る
        -- print("Icon already exists for AID: " .. aid) -- デバッグ用
        return
    end
    -- ★★★ ここまで修正 ★★★
    ...
    if type == PARTY_GUILD then
        header = "GM_"
        -- ギルドメンバーの場合、パーティアイコン(PM_)が既にあれば、ギルドアイコン(GM_)は作らない、という仕様にするならここで return する手もある
        -- if gbox:GetChild("PM_" .. aid) ~= nil then return end
    else -- PARTY_NORMAL の場合
        -- パーティメンバーの場合、ギルドアイコン(GM_)が既にあれば、パーティアイコン(PM_)は作らない、という仕様にするならここで return する手もある
        -- if gbox:GetChild("GM_" .. aid) ~= nil then return end
    end
    local name = header .. aid

    if pcInfo:GetMapID() == 0 then
        -- mapIDが0なら関連アイコン削除(これは現状維持でよさそうだけど、gboxから消すのがベター)
        local existing_pm = gbox:GetChild("PM_" .. aid)
        if existing_pm then
            gbox:RemoveChild("PM_" .. aid)
        end
        local existing_gm = gbox:GetChild("GM_" .. aid)
        if existing_gm then
            gbox:RemoveChild("GM_" .. aid)
        end
        return
    end
