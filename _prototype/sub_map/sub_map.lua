...
function sub_map_GET_MAP_POS(frame, mapprop, x, y, width, height)

    local map_pic = frame:GetChildRecursively("map_pic")

    local cursize = GET_MINIMAPSIZE();
    local pictureui = GET_CHILD(frame:GetTopParentFrame(), "map", "ui::CPicture");
    local minimapw = map_pic:GetImageWidth() * (100 + map_pic:GetWidth()) / 100;
    local minimaph = map_pic:GetImageHeight() * (100 + map_pic:GetHeight()) / 100;
...
function sub_map__MAP_MON_MINIMAP_SETPOS(frame, handle, x, z)
    ...
    local XC, YC, sx, sy = sub_map_GET_MAP_POS(frame, mapprop, x, z, width, height);
...
function sub_map_MAP_MON_MINIMAP_SETPOS(frame, info)

    sub_map__MAP_MON_MINIMAP_SETPOS(frame, info.handle, info.x, info.z);
...
function sub_map_MAP_MON_MINIMAP(frame, msg, argStr, argNum, info)
    ...
    local monPic = frame:GetChild(ctrlName);
    if monPic ~= nil then
        sub_map_MAP_MON_MINIMAP_SETPOS(frame, info);
        return
    end
    ...
    if monCls.MonRank == "Boss" then
        ...
        if false == monPic:HaveUpdateScript("sub_map__MONPIC_AUTOUPDATE") then
            monPic:RunUpdateScript("sub_map__MONPIC_AUTOUPDATE", 0);
        end
...
function sub_map__MONPIC_AUTOUPDATE(ctrl)
    ...
    if actor ~= nil then
        ...
        sub_map_MAP_MON_MINIMAP_SETPOS(ctrl:GetParent(), handle, actorPos.x, actorPos.z);
        return 1;
    end
