...
function g.mkdir_new_folder()
    ...
    if not session or not session.loginInfo or not session.loginInfo.GetAID then
        g.active_id = "UNKNOWN_AID"
    else
    ...
    -- ヘルパー関数たち
    local function unicode_to_codepoint(char) -- <--- コレね！
        local codepoint = utf8.codepoint(char)
        return string.format("%X", codepoint)
    end
    ...
    local function convert_to_ascii(input_str)
        local result = ""
        if utf8 and utf8.charpattern and utf8.codepoint then -- utf8.codepoint もチェックしとくね
            for char_code in input_str:gmatch(utf8.charpattern) do
                result = result .. unicode_to_codepoint(char_code) -- ここで使う！
            end
        else
            result = input_str:gsub("[^%w%s]", "_")
        end
        return result
    end
    ...
    -- ファミリーネームから作るファイルパス候補
    local family_name_input = GETMYFAMILYNAME() and GETMYFAMILYNAME() or ""
    local family_name_ascii = convert_to_ascii(family_name_input) -- ここで convert_to_ascii が呼ばれる
    local current_addon_name = addonNameLower or "unknown_addon"
    ...
    -- 1. メイン設定 (g.settings と g.settings_path) の準備
    local settings_data
    ...
    if settings_file_check then
        ...
        local copied_from_change = copy_file_if_exists(potential_change_file_loc, g.settings_path)
        if not copied_from_change then
            copy_file_if_exists(potential_settings_file_loc, g.settings_path)
        end
        settings_data = load_json(g.settings_path)
    end
    ...
    -- 2. 'change' 設定 (g.change_settings と g.change_path) の準備
                ...
                if i <= 4 then
                    ...
                    if skill ~= nil then
                        ...
                        local skill_name = gbox:CreateOrGetControl("richtext",
                                                                   "skill_name" .. equipType .. character.name,
