...
function GODDESS_EQUIP_UI_TUTORIAL_CHECK(frame, msg, arg_str, arg_num)
    ...
    if session.shop.GetEventUserType() == 0 then
        return
    end
    ...
    if guid ~= 'None' then
        ...
        FOR_EACH_INVENTORY(inv_item_list, function(inv_item_list, inv_item, slotset, use_lv)
            ...
            if arg_str ~= 'NO' then
                ...
                if arg_str == 'normal' then
                    local lv_txt = slot:CreateOrGetControl('richtext', 'lv_txt', 0, 0, slot:GetWidth(),
                        slot:GetHeight() * 0.3)
...
function ON_SUCCESS_REFORGE_REINFORCE_EXEC(frame, msg, arg_str, arg_num)
    ...
    local success_scp = string.format('RESULT_EFFECT_UI_RUN_SUCCESS(\'%s\', \'%s\', \'%d\', \'%d\', %d)',
        '_END_REFORGE_REINFORCE_EXEC', icon, left, top, high_grade)
...
function ON_FAILED_REFORGE_REINFORCE_EXEC(frame, msg, arg_str, arg_num)
    ...
    local failed_scp = string.format('RESULT_EFFECT_UI_RUN_FAILED(\'%s\', \'%d\', \'%d\')',
        '_END_REFORGE_REINFORCE_EXEC', left, top)
...
local function _GODDESS_MGR_MAKE_ENCHANT_OPTION(box, item_obj)
    ...
    if item_obj ~= nil then
        ...
        if rareOptionText ~= nil then
            local rareOptionCtrl = box:CreateOrGetControlSet('eachproperty_in_itemrandomreset', 'PROPERTY_CSET_RARE', 0,
                0)
...
function _UPDATE_GODDESS_TRANSCEND_RESULT(frame)
    ...
    for i = 1, #prop_name_list do
        ...
        local ctrlSet = result_bg:CreateOrGetControlSet('transcend_result_text', 'RV_' .. prop_name, ui.CENTER_HORZ,
            ui.TOP, 0, 0, 0, 0)
...
function GODDESS_MGR_REFORGE_TRANSCEND_EXEC(parent, btn)
    ...
    local msgbox = ui.MsgBox(ScpArgMsg('DoGoddessTranscend', 'count', need_count), yesscp,
        'ENABLE_CONTROL_WITH_UI_HOLD(false)')
    SET_MODAL_MSGBOX(msgbox)
end
...
function GODDESS_MGR_SOCKET_NORMAL_UPDATE(frame)
    ...
    if guid ~= 'None' then
        ...
        for i = 0, max_socket_cnt - 1 do
            local ctrlset = normal_inner_bg:CreateOrGetControlSet('eachsocket_in_goddessmgr', 'NORMAL_CSET_' .. i, 5,
                i * 90)
...
function GODDESS_MGR_SOCKET_OPEN(frame)
    INVENTORY_SET_CUSTOM_RBTNDOWN('INVENTORY_SET_CUSTOM_RBTNDOWN')
    GODDESS_MGR_SOCKET_CLEAR(frame)
end
...
function GODDESS_MGR_SOCKET_REQ_NORMAL_ENABLE(parent, btn)
    ...
    if guid ~= 'None' then
        ...
        local list = item_goddess_socket.get_normal_socket_material_list(lv, TryGetProp(item_obj, 'ClassType', 'None'),
            index)
...
local function GET_GODDESS_MAKE_SHOW_OPTION(frame)
    ...
    return group_list:GetUserValue('GROUP_INDEX_' .. group_list:GetSelItemIndex()),
        ...
        type_list:GetUserValue('GROUPNAME_INDEX_' .. type_list:GetSelItemIndex()), only_enable_equip:IsChecked(),
...
local function IS_NEED_TO_SHOW_GODDESS_RECIPE(frame, recipeCls, checkGroup, checkMaterial, checkGroupName,
    checkEquipable, checkHaveMaterial)
...
local function GODDESS_MAKE_MAKE_CTRLSET(recipeBox, recipeCls, checkGroup, checkMaterial, checkGroupName,
    checkEquipable, checkHaveMaterial)
    local frame = ui.GetFrame('goddess_equip_manager')
    if IS_NEED_TO_SHOW_GODDESS_RECIPE(frame, recipeCls, checkGroup, checkMaterial, checkGroupName, checkEquipable,
        checkHaveMaterial) == false then
        return
    end
    ...
    -- material
    local matBox = ctrlset:CreateControl('groupbox', 'matBox', itemIcon:GetX() + itemIcon:GetWidth(), itemIcon:GetY(),
        500, 150)
...
function GODDESS_MGR_MAKE_MAKE_LIST(frame)
    ...
    local checkGroup, checkMaterial, checkGroupName, checkEquipable, checkHaveMaterial = GET_GODDESS_MAKE_SHOW_OPTION(
        frame)
    local list = GET_GODDESS_MAKE_TARGET_LIST(frame)
    if list ~= nil then
        for i = 1, #list do
            local cls = GetClass('goddessrecipe', list[i])
            GODDESS_MAKE_MAKE_CTRLSET(list_gb, cls, checkGroup, checkMaterial, checkGroupName, checkEquipable,
                checkHaveMaterial)
        end
...
function PLAY_GODDESS_MAKE_SUCCESS_EFFECT(frame, msg, item_name, recipe_id)
    ...
    movie.PlayUIEffect(bg_frame:GetUserConfig('BLACKSMITH_RESULT_EFFECT'), screenWidth / 2, screenHeight / 2,
        tonumber(bg_frame:GetUserConfig('BLACKSMITH_RESULT_EFFECT_SCALE')))
...
local function GODDESS_MGR_MAKE_INHERIT_TARGET_LIST(gbox, inv_item, item_obj)
    ...
    for i = 1, #list do
        ...
        local ctrlset = gbox:CreateOrGetControlSet('eachitem_in_exchange_weapontype',
            'INHERIT_WEAPONTYPE_CSET_' .. (i - 1), 0, (i - 1) * 90)
        if ctrlset ~= nil then
...
function WARNINGMSGBOX_FRAME_INHERIT()
    ...
    if from_frame:IsVisible() == 0 then
        return
    end
    ...
    local totalHeight = warningbox:GetY() + warningText:GetY() + warningText:GetHeight() + noBtn:GetHeight() + 2 *
                            buttonMargin.bottom
...
local function GODDESS_MGR_MAKE_CONVERT_TARGET_LIST(gbox, inv_item, item_obj)
    ...
    for i = 1, #list do
        ...
        local ctrlset = gbox:CreateOrGetControlSet('eachitem_in_exchange_weapontype',
            'CONVERT_WEAPONTYPE_CSET_' .. (i - 1), 0, (i - 1) * 90)
        if ctrlset ~= nil then
            local icon = GET_CHILD_RECURSIVELY(ctrlset, 'item_icon', 'ui::CPicture')
            icon:ShowWindow(1)
            icon:SetImage(cls.Icon)
...
function GODDESS_MGR_CONVERT_MAT_LIST_UPDATE(frame)
    ...
    if TryGetProp(target_cls, "EquipGroup", "None") == "Weapon" or TryGetProp(target_cls, "EquipGroup", "None") ==
        "THWeapon" or TryGetProp(target_cls, "EquipGroup", "None") == "SubWeapon" then
        taget_is = 'Weapon'
    elseif TryGetProp(target_cls, "ClassType", "None") == "Neck" or TryGetProp(target_cls, "ClassType", "None") ==
        "Ring" then
        taget_is = 'Acc'
    end
    ...
    if nameList ~= nil and countList ~= nil and #nameList > 0 and #countList > 0 then
        for i = 1, #nameList do
            local ctrlSet = mat_list_bg:CreateOrGetControlSet('eachmaterial_in_exchangeantique',
                'CONVERT_WEAPONTYPE_MAT_CSET' .. i, 20, (i - 1) * 40)
            if ctrlSet ~= nil then
