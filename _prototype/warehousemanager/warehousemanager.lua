...
WarehouseManager.ischeck = 0
function WAREHOUSEMANAGER_INIT_FRAME()

    local frame = ui.GetFrame("warehousemanager")
    local checkbox = frame:CreateOrGetControl('checkbox', 'addoncheck', 410, 200, 25, 25)
    AUTO_CAST(checkbox)
    checkbox:SetCheck(WarehouseManager.ischeck)
    checkbox:SetEventScript(ui.LBUTTONUP, "WAREHOUSEMANAGER_ISCHECK")
    checkbox:ShowWindow(1)

end

function WAREHOUSEMANAGER_ISCHECK(frame, ctrl, argStr, argNum)

    local ischeck = ctrl:IsChecked();

    if ischeck == 1 then
        WarehouseManager.ischeck = 1
        print(WarehouseManager.ischeck)
    else
        WarehouseManager.ischeck = 0
        print(WarehouseManager.ischeck)
    end
...
function WAREHOUSEMANAGER_ON_INIT(addon, frame)
    ...
    if WarehouseManager.ischeck == 0 then
