...
function CHARACTERS_ITEM_SERCH_ON_INIT(addon, frame)
    ...
    local function get_localized_tooltip(lang)
        if lang == "Japanese" then
            return "{@st59}インベントリ (F2){nl}Right click: Characters_Item_Serch"
        elseif lang == "kr" then
            return "{@st59}인벤토리 (F2){nl}Right click: Characters_Item_Serch"
        else
            return "{@st59}Inventory (F2){nl}Right click: Characters_Item_Serch"
        end
    end
