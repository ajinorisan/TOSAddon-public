...
function EASYBUFF_CMD(command)
    if g.settings.useHook == 1 then
        ...
        CHAT_SYSTEM(addonNameLower .. " off")
    else
        g.settings.useHook = 1
        EASYBUFF_SAVESETTINGS();
        CHAT_SYSTEM(addonNameLower .. " on")
    end
    return;
end

function EASYBUFF_FRAME_INIT()
    ...
    local btn2 =
        frame:CreateOrGetControl('button', 'myButton2', 140, 50, 80, 30)
    btn2:SetText('{ol}4foodonly')
    btn2:SetEventScript(ui.LBUTTONUP, 'EASYBUFF_4FOODONLY')

    local btn3 =
        frame:CreateOrGetControl('button', 'myButton3', 240, 50, 80, 30)
    btn3:SetText('{ol}5foodonly')
    btn3:SetEventScript(ui.LBUTTONUP, 'EASYBUFF_5FOODONLY')

    local btn4 =
        frame:CreateOrGetControl('button', 'myButton4', 340, 50, 80, 30)
...
function EASYBUFF_ON_INIT(addon, frame)
    ...
    addon:RegisterMsg('GAME_START_3SEC', 'EASYBUFF_FRAME_INI')
...
function EASYBUFF_SQUIRE_BUFF_EXCUTE()
    -- CHAT_SYSTEM("test")
    local enable_slot_list = {'RH', 'LH', 'RH_SUB', 'LH_SUB', 'SHIRT', 'PANTS',
                              'GLOVES', 'BOOTS'}
    ...
    for i = 1, #enable_slot_list do
        ...
        local ctrlset = GET_CHILD_RECURSIVELY(frame,
                                              'ITEMBUFF_CTRL_' .. slot_name)
        if ctrlset ~= nil then
            local checkbox = GET_CHILD(ctrlset, 'checkbox')
            checkbox:SetCheck(1)
            if checkbox:IsChecked() == 1 then
                local inv_item = session.GetEquipItemBySpot(
                                     item.GetEquipSpotNum(slot_name))
    ...
    -- frame:ShowWindow(0)
    ui.SysMsg(
        "{#FFFFFF}Equipment Maintenance in progress. To cancel, use the Cancel button.")
    ui.SysMsg(
        "{#FFFFFF}装備メンテナンス中 キャンセルは取消ボタンで!")
    session.autoSeller.BuyItems(handle, AUTO_SELL_SQUIRE_BUFF,
                                session.GetItemIDList(), skillName)
...
function EASYBUFF_ONBUTTON()
    ...
    g.buffIndex = 0;
    ReserveScript(string.format('EASYBUFF_BUY(%d, %d, %d)', handle, sellType,
                                cnt), 0.5)
end
...
function EASYBUFF_BUY(handle, sellType, cnt)
    if g.buffIndex < cnt then
        ...
        ReserveScript(string.format('EASYBUFF_BUY(%d, %d, %d)', handle,
                                    sellType, cnt), 0.5)
    else
...
function EASYBUFF_ONBUTTON_REPAIR()
    ...
    for i = 0, slotCount - 1 do
        ...
        if slot:GetIcon() ~= nil then
            ...
            local needItem, needCount = ITEMBUFF_NEEDITEM_Squire_Repair(
                                            GetMyPCObject(), itemobj);
...
function EASYBUFF_HOOK_CLICK_FOOD(groupName, sellType, handle, sellerCID,
                                  arg_num)
