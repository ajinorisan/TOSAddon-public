...
function skill_notice_free_effect_test(effectName, sound, size)
...
function skill_notice_free_buffid_edit(frame, buffid_edit, frame_name, index, no_save)
...
function skill_notice_free_setting(frame, ctrl, str, num)
    ...
    local function create_slot_and_edit(index, y, buff_id)
        ...
        if buff_id then
            local no_save = true
            skill_notice_free_buffid_edit(frame, buffid_edit, setting_frame:GetName(), index, no_save)
        end
    end
        ...
        if cid_table[str_buff_id] == "YES" then
            ...
            if mode == "icon" then
                ...
                if buff == nil then
                    ...
                    buff_count:SetText("{ol}{s35}" .. buff.over)
                end
...
function skill_notice_free_buff_remove(frame, msg, str, buff_type)
    local frame = ui.GetFrame("skill_notice_free")
    local my_handle = session.GetMyHandle()
    ...
    local buff_class = GetClassByType('Buff', tonumber(str_buff_type))
    if buff_class == nil then
        return
    end
    ...
    if buff_data.mode == "icon" then
        ...
        if icon_frame then
            local icon_slot = GET_CHILD_RECURSIVELY(icon_frame, "icon_slot" .. str_buff_type)
            if icon_slot then
                AUTO_CAST(icon_slot)
                local buff_count = GET_CHILD_RECURSIVELY(icon_slot, "buff_count" .. str_buff_type)
                AUTO_CAST(buff_count)
                if buff_count then
                    buff_count:SetText("{ol}{s35}0")
                    buff_count:SetColorTone("FFFFFFFF")
        ...
        if gauge then
            ...
            local color = buff_table[str_buff_type].color
            gauge:SetColorTone(color)
        ...
        g.buffs[str_buff_type] = nil
    end

end

function skill_notice_free_buff_add(frame, msg, str, buff_type)
    local frame = ui.GetFrame("skill_notice_free")
    local my_handle = session.GetMyHandle()
    -- local actor = world.GetActor(myHandle)
    ...
    local buff_data = buff_table[str_buff_type]
    local buff_class = info.GetBuff(my_handle, tonumber(str_buff_type))
    ...
    if buff_data.mode == "icon" then
        ...
        if icon_frame then
            ...
            if icon_slot then
                ...
                AUTO_CAST(buff_count)
                if buff_count then
                    buff_count:SetText("{ol}{s35}" .. buff_class.over)
                    buff_count:SetColorTone("FFFFFFFF")
                end
    ...
    g.buffs[str_buff_type] = false

end

function skill_notice_free_buff_update(frame, msg, str, buff_type)
...
function skill_notice_free_apply_buff_effects(buff_data)
    ...
    if effect_name and effect_name ~= "None" then
        ...
        -- effect.DetachActorEffect(actor, effect_name, 10.0);
    end
...
--[[function skill_notice_free_new_buff_frame()
    ...
    noticeframe:SetEventScript(ui.LBUTTONUP, "skill_notice_free_end_drag")
...
function skill_notice_free_ischeck(frame, ctrl, argStr, argNum)
    local ischeck = ctrl:IsChecked()
    ...
    if ctrlname == "new_check" then
        ...
        if ischeck == 1 then
            skill_notice_free_new_buff_frame()
    ...
    skill_notice_free_save_settings()
    skill_notice_free_frame_init(frame)
end

function skill_notice_free_effect_size(frame, ctrl, argStr, argNum)
    ...
    skill_notice_free_save_settings()
    ReserveScript(string.format("skill_notice_free_effect_test_size('%s',%d)", argStr, size), 0.1)

end

function skill_notice_free_effect_test_size(argStr, size)
    local myHandle = session.GetMyHandle();
    local actor = world.GetActor(myHandle)
    local effectName
    local sound
