...
function YAIC_INIT()
    EBI_try_catch {
        try = function()
            ...
            local gbox = frame:CreateOrGetControl("groupbox", "gboxkeybinds", 5, 300, frame:GetWidth() - 10,
                frame:GetHeight() - 350)
            AUTO_CAST(gbox)
            ...
            local editinterval = frame:CreateOrGetControl("edit", "editinterval", 200, 100, 80, 30)
            AUTO_CAST(editinterval)
            editinterval:SetText(tostring(g.settings.speed))
            editinterval:SetSkinName("systemmenu_vertical")
            editinterval:SetFontName("white_16_ol")
            editinterval:SetTextAlign("center", "center")
            local label = frame:CreateOrGetControl("richtext", "labellimit", 20, 140, 80, 30)
            label:SetText("{ol}" .. L_("Stack limit"))
            local editlimit = frame:CreateOrGetControl("edit", "editlimit", 200, 140, 80, 30)
            AUTO_CAST(editlimit)
            editlimit:SetText(tostring(g.settings.stacklimit))
            editlimit:SetSkinName("systemmenu_vertical")
            editlimit:SetFontName("white_16_ol")
            editlimit:SetTextAlign("center", "center")
            ...
            local button = frame:CreateOrGetControl("button", "btnsave", frame:GetWidth() - 100, frame:GetHeight() - 40,
                80, 30)
            AUTO_CAST(button)
...
function YAIC_ADD_KEYBINDCONTROL(parent, index, keybind, offsety)
    return EBI_try_catch {
        try = function()
            local h = 80
            local gbox = parent:CreateOrGetControl("groupbox", "gboxkeybind" .. tostring(index), 5, offsety,
                parent:GetWidth() - 30, h)
            AUTO_CAST(gbox)
