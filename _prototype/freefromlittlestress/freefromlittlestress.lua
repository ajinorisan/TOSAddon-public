...
function FREEFROMLITTLESTRESS_LOADSETTINGS()
    ...
    for loginCharID, v in pairs(g.settings) do
        g.settings.charid[loginCharID] = v
        if v == 1 then
            g.check = 1
        else
            g.check = 0
...
function FREEFROMLITTLESTRESS_ON_INIT(addon, frame)
    ...
    g.SetupHook(FREEFROMLITTLESTRESS_OPEN_COMPANIONLIST, "ON_OPEN_COMPANIONLIST")
...
function FREEFROMLITTLESTRESS_OPEN_COMPANIONLIST()
...
function FREEFROMLITTLESTRESS_ON_OPEN_COMPANIONLIST()
    local frame = ui.GetFrame("companionlist");
    frame:ShowWindow(1);
    -- frame:SetGravity(ui.CENTER, ui.CENTER);
    -- frame:SetMargin(-500, -200, 350, 70);
