...
-- v1.2.5 メモ欄バグってたの修正
-- v1.2.6 再修正。。。
-- v1.2.7 instantccと共存しようとしてたから、バラックレイヤー取得部分がバグってたので、機能自体を移植した。
...
g.settings_file_location = string.format('../addons/%s/%s.json', addonNameLower, active_id)
...
function indun_list_viewer_sort_characters(account_info)
    ...
    for order = 0, layer_pc_count - 1 do
        ...
        g.settings[pc_name].gender = gender
        g.settings[pc_name].level = level
        g.settings[pc_name].jobid = jobid
...
function indun_list_viewer_enable_layer_btn(frame)
    local frame = ui.GetFrame('barrack_charlist')
    local layer = tonumber(frame:GetUserValue("SelectBarrackLayer"))
    g.layer = layer
end
...
function INDUN_LIST_VIEWER_ON_INIT(addon, frame)
    ...
    if type(_G[functionName]) == "function" then
        ...
        _G["INSTANTCC_BARRACK_START_FRAME_OPEN"] = nil
        _G["INSTANTCC_BARRACK_TO_GAME"] = nil
    end
...
function indun_list_viewer_BARRACK_START_FRAME_OPEN(frame)
    indun_list_viewer_BARRACK_START_FRAME_OPEN_(frame)
end

function indun_list_viewer_BARRACK_START_FRAME_OPEN_(frame)
    if frame == nil then
        return
    end
    ...
    if g.do_cc and g.try <= 1 then
        g.retry = 0
        ReserveScript("indun_list_viewer_CHANGE()", 0.1)
    end
end

function indun_list_viewer_CHANGE()
    barrack.SelectBarrackLayer(g.do_cc.layer)
    ReserveScript(string.format("barrack.SelectCharacterByCID('%s')", g.do_cc.cid), 0.1)
    ReserveScript("indun_list_viewer_TOGAME()", 0.2)
end

function indun_list_viewer_RETRY()
    g.retry = g.retry + 1
    ...
    indun_list_viewer_CHANGE()
end

function indun_list_viewer_TOGAME()
...
function indun_list_viewer_BARRACK_TO_GAME_()
...
function indun_list_viewer_DO_CC(cid, layer)
...
function indun_list_viewer_APPS_TRY_MOVE_BARRACK_()

    local context = ui.CreateContextMenu("indun_list_viewer_SELECT_CHARACTOR", "Barrack Charactor List", 0, 0, 0, 0)
    ui.AddContextMenuItem(context, "Return To Barrack", "indun_list_viewer_DO_CC()")
    for i = 1, #g.sorted_settings do

        local info = g.sorted_settings[i]
        ...
        ui.AddContextMenuItem(context, str, string.format("indun_list_viewer_DO_CC('%s',%d)", info.cid, info.layer))
    end
...
function indun_list_viewer_job_slot(frame, data, y)
    ...
    job_slot:SetEventScript(ui.LBUTTONDOWN, "indun_list_viewer_DO_CC_")
    ...
    name_text:SetEventScript(ui.LBUTTONDOWN, "indun_list_viewer_DO_CC_")
...
function indun_list_viewer_DO_CC_(frame, ctrl, cid, layer)
    g.layer = nil
    indun_list_viewer_DO_CC(cid, layer)
end
