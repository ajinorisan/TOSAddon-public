...
if option.GetCurrentCountry() == "Japanese" then
    soundTypes = {
        [1] = {
            name = 'button_click_stats_up',
            sound = 'button_click_stats_up'
        },
        [2] = {
            name = 'quest_count',
            sound = 'quest_count'
        ...
        [3] = {
            name = 'quest_event_start',
            sound = 'quest_event_start'
        },
        [4] = {
            name = 'quest_success_2',
            sound = 'quest_success_2'
        },
        [5] = {
            name = 'sys_alarm_mon_kill_count',
            sound = 'sys_alarm_mon_kill_count'
        },
        [6] = {
            name = 'quest_event_click',
            sound = 'quest_event_click'
        },
        [7] = {
            name = 'sys_secret_alarm',
            sound = 'sys_secret_alarm'
        },
        [8] = {
            name = 'travel_diary_1',
            sound = 'travel_diary_1'
        },
        [9] = {
            name = 'button_click_4',
            sound = 'button_click_4'
        },
        [10] = {
            name = 'うぅーい！',
            sound = 'voice_archer_multishot_cast'
        },
        [11] = {
            name = 'はいっ！',
            sound = 'voice_archer_camouflage_shot'
        },
        [12] = {
            name = 'このぉ！',
            sound = 'voice_archer_cloaking_shot'
        },
        [13] = {
            name = 'そぉい！',
            sound = 'voice_archer_fulldraw_cast'
        },
        [14] = {
            name = 'フィーバー！',
            sound = 'voice_war_jollyroger_shot'
        },
        [15] = {
            name = 'あーあー',
            sound = 'PLAYLIST_m_boss_scenario2'
        }
    }
else
    soundTypes = {
...
function CHATEXTENDS_SOUND_SAVE_SETTINGS()
    ...
    if g.settings.all.normal or g.settings.all.shout or g.settings.all.party or g.settings.all.guild or
        g.settings.all.whisper or g.settings.all.group or g.settings.all.system then
        g.settings.flg = true;
    end
    ...
    for i, ver in ipairs(g.settings.word) do
        if g.settings.word[i].normal or g.settings.word[i].shout or g.settings.word[i].party or g.settings.word[i].guild or
            g.settings.word[i].whisper or g.settings.word[i].group or g.settings.word[i].system then
            g.settings.flg = true;
        end
...
function CHATEXTENDS_SOUND_FRAME_CREATE()
    ...
    for i, ver in ipairs(rtLabel) do
        local header = frame:CreateOrGetControl("richtext", "chatsounds_label" .. i, rtLabel[i].left, rtLabel[i].top,
            rtLabel[i].h, rtLabel[i].w);
    ...
    for i = 0, 6 do
        ...
        for j, ver in ipairs(rtAll) do
            local create_CTRL = frame:CreateOrGetControl(rtAll[j].type, rtAll[j].name .. i, rtAll[j].left + marginLeft,
                rtAll[j].top, rtAll[j].h, rtAll[j].w);
            ...
            elseif rtAll[j].type == "droplist" then
                ...
                for k, v in ipairs(soundTypes) do
                    create_CTRL:AddItem(k, string.format("{#FFFFFF}{ol}{b}{s16}%s", v.name), 0,
                        "CHAT_SOUNDS_PLAYSOUND('" .. v.sound .. "')");
                end
                -- ロード値のセット
                if i == 0 then
    ...
    for i = 1, 15 do
        ...
        for j, ver in ipairs(rtWord) do
            local create_CTRL = frame:CreateOrGetControl(rtWord[j].type, rtWord[j].name .. i, rtWord[j].left,
                rtWord[j].top + marginTop, rtWord[j].h, rtWord[j].w);
