...
function mini_addons_COLONY_WAR_RESTART_UPDATE(my_frame, my_msg)
...
-- CHAT_SYSTEM("!@#$Dead{MEMBER}$*$MEMBER$*$あじのり#@!")
-- CHAT_SYSTEM("!@#$Resurrect{MEMBER}$*$MEMBER$*$あじのり#@!")
...
function MINI_ADDONS_ON_INIT(addon, frame)
    ...
    if g.settings.restart_move == 1 then
        ...
        g.setup_hook_and_event(addon, "COLONY_WAR_RESTART_UPDATE", "mini_addons_COLONY_WAR_RESTART_UPDATE", false)
        g.mouse = false
    end
...
function MINI_ADDONS_subframe_open(frame, ctrl, str)
    ...
    if ctrl_name == "chats" then
        ...
        for i, setting in ipairs(settings) do
            local checkbox = gbox:CreateOrGetControl('checkbox', setting.name .. "_checkbox", 10, y, 25, 25)
            AUTO_CAST(checkbox)
...
function MINI_ADDONS_SETTING_FRAME_INIT(frame, ctrl, str, num)
    ...
    for i, setting in ipairs(settings) do
        ...
        if setting.name == "party_buff" then
            ...
            local pt_buff = gbox:CreateOrGetControl('checkbox', "pt_buff" .. "_checkbox", text_width + 15 + 70, y - 5,
                25, 25)
            AUTO_CAST(pt_buff)
