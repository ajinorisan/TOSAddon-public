...
function MINI_ADDONS_LANG(str)
    ...
    if langcode == "Japanese" then
        ...
        if str == "Perfect and Black Market notices not displayed in chat" then
            str = "パーフェクトとブラックマーケットのお知らせをチャットに表示しない"
        end
        ...
        if str == "Reduces CC errors during continuous hard raids" then
            str = "連続ハードレイド中のキャラクターチェンジ間違いを軽減します"
        end

        if str == "Switching the display of the party info frame. For mouse mode.Party info right-click" then
            str = "パーティーフレームの表示切替。マウスモード用"
        end
...
function MINI_ADDONS_SETTING_FRAME_INIT()
    ...
    raid_check:SetText("{ol}{#FF4500}" .. MINI_ADDONS_LANG("Reduces CC errors during continuous hard raids"))
    raid_check:SetTextTooltip("연속 하드 레이드 중 CC 실수를 줄일 수 있습니다.")
