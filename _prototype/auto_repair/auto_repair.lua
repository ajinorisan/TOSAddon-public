...
function AUTO_REPAIR_ON_INIT(addon, frame)
    ...
    if map_class.MapType == "City" then
        ...
        local auto_repair_item = session.GetInvItemByName('QuestReward_repairPotion_510')
...
function AUTO_REPAIR_SETTING_FRAME_INIT()
    ...
    AUTO_CAST(text_1)
    local item_class = GetClassByType('Item', 11200365)
...
function AUTO_REPAIR_BUY()
    ...
    local auto_repair_item = session.GetInvItemByName('QuestReward_repairPotion_510')
    local shop_type = "RadaCertificate"
    local recipe_class = GetClass("ItemTradeShop", 'RadaCertificate_13')
    local count = 0
...
function AUTO_REPAIR_ITEM_USE(obj, spot)
    ...
    local repair_potions = {session.GetInvItemByName('EVENT_2005_repairPotion'),
                            session.GetInvItemByName('Premium_repairPotion'),
                            session.GetInvItemByName('QuestReward_repairPotion_510')}

    for _, potion in ipairs(repair_potions) do
        if potion ~= nil and not potion.isLockState then
            local repeatCount = math.min(potion.count, 4)
            for i = 0, repeatCount - 1 do
                if obj.Dur / obj.MaxDur < 0.9 then
                    item.UseByGUID(potion:GetIESID())
                else
                    return
                end
            end
