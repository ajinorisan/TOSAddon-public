...
function indun_panel_init(frame)
    ...
    cc:SetText("{img barrack_button_normal 30 30}")
    cc:SetEventScript(ui.LBUTTONUP, "APPS_TRY_MOVE_BARRACK")
    cc:SetTextTooltip("バラックに戻ります。{nl}Return to Barracks.")

    local shop = frame:CreateOrGetControl("button", "shop", 155, 10, 25, 25)
    AUTO_CAST(shop)
    shop:SetSkinName("None")
    shop:SetText("{img icon_item_Tos_Event_Coin 23 23}")
    shop:SetTextTooltip("TOSイベントショップ{nl}TOS Event Shop")
    shop:SetEventScript(ui.LBUTTONUP, "indun_panel_event_tos_whole_shop_open")

    local config = frame:CreateOrGetControl('button', 'config', 120, 5, 30, 35)
    AUTO_CAST(config)
    config:SetSkinName("None")
    config:SetText("{img config_button_normal 30 30}")
    config:SetEventScript(ui.LBUTTONUP, "indun_panel_config_gb_open")
    config:SetTextTooltip("レイド表示設定{nl}Raid Display Settings")

    if config:IsVisible() == 1 then
    ...
    for i = 1, count do
        ...
        for key, value in pairs(entry) do
            if g.settings[key .. "_checkbox"] == 1 then
                local indunText = frame:CreateOrGetControl("richtext", key, x - 125, y + 5)
                indunText:SetText("{ol}{#FFFFFF}" .. INDUN_PANEL_LANG(key))
                indunText:AdjustFontSizeByWidth(120)
