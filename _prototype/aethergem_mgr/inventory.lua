...
g_invenTypeStrList = {"All", "Equip", "Consume", "Recipe", "Card", "Etc", "Gem", "Premium", "Housing", "Pharmacy",
                      "Quest"};
...
function INVENTORY_WEIGHT_UPDATE(frame)
    ...
    local weightscptext = ScpArgMsg("Weight{All}{Max}", "All", string.format("%.1f", pc.NowWeight), "Max",
        string.format("%.1f", pc.MaxWeight))
...
function TRY_TO_USE_WARP_ITEM(invitem, itemobj)
    ...
    -- 워프 주문서 아니면 리턴
    if itemobj.ClassID ~= 640022 and itemobj.ClassID ~= 640079 and itemobj.ClassID ~= 490006 and itemobj.ClassID ~=
        490110 then
        return 0;
    end
    ...
    if IsBuffApplied(pc, 'WEEKLY_BOSS_RAID_BUFF') == 'YES' or IsBuffApplied(pc, 'Event_Penalty') == 'YES' or
        IsBuffApplied(pc, 'PVP_MINE_BUFF1') == 'YES' or IsBuffApplied(pc, 'PVP_MINE_BUFF2') == 'YES' then
        ui.SysMsg(ScpArgMsg("CannotUseThieInThisMap"));
        return 0;
    end
...
function IS_TEMP_LOCK(invFrame, invitem)
    if invFrame:GetUserValue('ITEM_GUID_IN_MORU') == invitem:GetIESID() or invitem:GetIESID() ==
        invFrame:GetUserValue("ITEM_GUID_IN_TRANSCEND") or invitem:GetIESID() ==
        invFrame:GetUserValue("ITEM_GUID_IN_TRANSCEND_SCROLL") then
        return true;
    end
...
function INVENTORY_RBDC_ITEMUSE(frame, object, argStr, argNum)
    ...
    if equip == true then
        ...
        if itemType == 'Consume' or itemType == 'Quest' or itemType == 'Cube' or groupName == 'ExpOrb' or groupName ==
            'SubExpOrb' then
            if itemobj.Usable == 'ITEMTARGET' then
                ...
                if itemType == 'Consume' and gachaCubeFrame ~= nil and gachaCubeFrame:IsVisible() == 1 and
                    invItemAllowReopen == 'YES' then
                    return
                end
    ...
    elseif legendcardupgradeFrame:IsVisible() == 1 and (itemobj.GroupName == "Card" or itemobj.ItemType == 'Etc') then
        ...
        if itemobj.ItemType == 'Etc' then
            ...
            for i = 0, cnt - 1 do
                ...
                if cardLv == legendCardLv and legendCardObj.CardGroupName ~= nil and legendCardObj.CardGroupName ==
                    'LEG' then
...
function DRAW_MEDAL_COUNT(frame)
    ...
    local tpText = ScpArgMsg("TPText{Premium}and{Event}", "Premium", tostring(accountObj.PremiumMedal), "Event",
        tostring(accountObj.GiftMedal))
...
function INVENTORY_ON_DROP(frame, control, argStr, argNum)
    ...
    elseif FromFrame:GetName() == "camp_ui" or FromFrame:GetName() == "warehouse" then
        ...
        if iconInfo.count > 1 then
            ...
            INPUT_NUMBER_BOX(toFrame, ScpArgMsg("InputCount"), "EXEC_TAKE_ITEM_FROM_WAREHOUSE", iconInfo.count, 1,
                iconInfo.count, nil, iesID);
        else
        ...
        if curtabIndex == 0 then
            ...
            if invItem ~= nil then
                if toicon == nil then
                    ...
                    icon:Set(frominfo:GetImageName(), 'THROWITEM', invItem.type, invItem.count, invItem:GetIESID(),
                        invItem.count);
...
function IS_EQUIPPED_WEAPON_SWAP_SLOT(invItem)
    ...
    return invItem:GetIESID() == slot1 or invItem:GetIESID() == slot2 or invItem:GetIESID() == slot3 or
               invItem:GetIESID() == slot4;
end

function STATUS_SLOT_DROP(frame, icon, argStr, argNum)
    local openFrame = ui.GetFrame("fragmentation_earring")
    if openFrame ~= nil and openFrame:IsVisible() == 1 then
        return
    end
...
function SORT_HAIR_COLORLIST(ctrl, colorKList, colorEList)
    ...
    local hairOrder = {"default", "black", "blue", "pink", "white", "blond", "red", "green", "gray", "lightsalmon",
                       "purple", "orange", "brown", "midnightblue"};
...
function CREATE_HAIR_DYE_DROPLIST(control, colorList, colorEList)
    local dropListFrame = ui.MakeDropListFrame(control, 0, 0, 150, 600, #colorList, ui.LEFT,
        "SELECT_HAIR_DYE_IN_DROPLIST", nil, nil);
...
function CREATE_DESIGNCUT_DYE_DROPLIST(control, colorList, colorEList)
    local dropListFrame = ui.MakeDropListFrame(control, 0, 0, 150, 600, #colorList, ui.LEFT,
        "SELECT_DESIGNCUT_DYE_IN_DROPLIST", nil, nil);
    for i = 0, #colorList do
        ui.AddDropListItem(colorList[i + 1], nil, colorEList[i + 1]);
    end
end
...
function INVENTORY_DELETE(itemIESID, itemType)
    ...
    local is_ark = TryGetProp(item_obj, 'GroupName', 'None') == 'Ark' and TryGetProp(item_obj, 'StringArg2', 'None') ==
                       'Made_Ark'
    ...
    if invItem.count > 1 then
        ...
        if item_grade >= 3 or warningMsgCostumeItem == true then
            if isGrowthEquip ~= true then
                clmsg = ScpArgMsg('HighItemGradeReallyDestroy{msg}{ITEM}', 'msg', ClMsg('destory_now'), 'ITEM',
                    s_dropDeleteItemName);
            end
        end
...
function CLIENT_CONVERT_TO_NOTRADE(item_obj)
    ...
    WARNINGMSGBOX_FRAME_OPEN(ScpArgMsg('ConvertToNoTradeWarning{NAME}', 'NAME', TryGetProp(item, 'Name', 'None')),
        yesscp, 'None', nil, ScpArgMsg('WarningTeamBelonging'));
end
...
function CHECK_EXEC_DELETE_ITEMDROP(count, className)
    ...
    if item_grade >= 3 or warningMsgCostumeItem == true then
        if isGrowthEquip ~= true then
            clmsg = ScpArgMsg('HighItemGradeReallyDestroy{msg}{ITEM}{COUNT}', 'msg', ClMsg('destory_now'), 'ITEM',
                s_dropDeleteItemName, 'COUNT', s_dropDeleteItemCount);
        end
    end
...
function UPDATE_INVENTORY_TOGGLE_ITEM(frame)
    ...
    for type, v in pairs(INVENTORY_TOGGLE_ITEM_LIST) do
        ...
        if slot ~= nil and slot:IsVisible() == 1 then
            ...
            if slot:GetDrawY() <= invenTab:GetDrawY() or invenTab:GetDrawY() + inventoryGbox:GetHeight() - offset <=
                slot:GetDrawY() then
                return 1;
            end
...
function SORT_ITEM_INVENTORY()
    ...
    elseif invTab:GetSelectItemName() == "tab_Gem" then
        scpScp = string.format("REQ_INV_SORT(%d, %d)", IT_INVENTORY, BY_LEVEL);
        ui.AddContextMenuItem(context, ScpArgMsg("SortByLevel"), scpScp);
    elseif invTab:GetSelectItemName() == "tab_Etc" or invTab:GetSelectItemName() == "tab_All" or
        invTab:GetSelectItemName() == "tab_Consume" then
...
function DO_WEAPON_SWAP(frame, index)
    ...
    SHOW_WEAPON_SWAP_TEMP_IMAGE(frame:GetUserIValue('CURRENT_WEAPON_RH'), frame:GetUserIValue('CURRENT_WEAPON_LH'),
        tempIndex)
end
...
function SCR_CHECK_SWAPABLE_C()
    if session.world.IsIntegrateIndunServer() == true then
        ...
        if mGameName ~= nil then
            ...
            if mGameName == 'LEGEND_RAID_MORINGPONIA_EASY' or mGameName == 'LEGEND_RAID_GLACIER_EASY' or
                string.find(mGameName, "CHALLENGE_AUTO") ~= nil or string.find(mGameName, "CHALLENGE_SOLO") ~= nil or
                mGameName == "CHALLENGE_DIVISION_AUTO" or mGameName == "LEGEND_RAID_GILTINE_AUTO" or mGameName ==
                "LEGEND_RAID_GILTINE_SOLO" or dungeonType == "MythicDungeon_Auto" then
                return false;
            end
        end
    end
...
function ON_UPDATE_TRUST_POINT(frame, msg, argStr, trustPoint)
    ...
    trustPointText:SetTextByKey("trustPoint",
        "{img icon_credit_grade_" .. trustPoint .. " 25 29}" .. " " .. trustPoint - 1);
...
function BEFORE_USE_QUEST_CLEAR_SCROLL(invItem)
    ...
    local textmsg =
        string.format("{#ff0000}[ %s ]{/}{nl}%s", itemobj.Name, ScpArgMsg("isrealUseQuestClearScroll_Msg_1"));
...
function BEFORE_APPLIED_YESSCP_OPEN_DO_NOT_TRADE_MSG(invItem)
    ...
    local textmsg =
        string.format("[ %s ]{nl}%s", itemobj.Name, ScpArgMsg("BEFORE_APPLIED_YESSCP_OPEN_DO_NOT_TRADE_MSG"));
    ui.MsgBox_NonNested(textmsg, itemobj.Name, 'REQUEST_SUMMON_BOSS_TX', "None");
...
function BEFORE_APPLIED_YESSCP_OPEN_LVCARD(invItem)
    ...
    if lv ~= 0 then
        ...
        local textmsg = string.format("[ %s ]{nl}%s", itemobj.Name, ScpArgMsg("EXPCARD_JUMPING_SET_LV_MSG",
            "BEFORELEVEL", beforeLV, "AFTERLEVEL", lv));
        ui.MsgBox_NonNested(textmsg, itemobj.Name, 'REQUEST_SUMMON_BOSS_TX', "None");
    end
    return;
end
...
function BEFORE_APPLIED_YESSCP_OPEN_EPISODE_UNLOCK(invItem)
    ...
    if lv ~= 0 then
        local textmsg =
            string.format("[ %s ]{nl}%s", itemobj.Name, ScpArgMsg("EXPCARD_JUMPING_SET_LV_MSG", "LEVEL", lv));
        ui.MsgBox_NonNested(textmsg, itemobj.Name, 'REQUEST_SUMMON_BOSS_TX', "None");
    end
    return;
end
...
function RUN_CLIENT_USE_MULTIPLE_MISC_PVP_MINE2(count)
    ...
    if now + getCount > maxValue then
        ...
        local invFrame = ui.GetFrame("inventory")
        invFrame:SetUserValue("REQ_USE_ITEM_GUID", invItem:GetIESID())
        local textmsg = string.format("[ %s ]{nl}%s", itemObj.Name,
            ScpArgMsg("IfLimitOver_misc_pvp_mine2_OVERUSE", "COUNT", calc))
...
function CLIENT_USE_MULTIPLE_USE_STRING_GIVE_ITEM_NUMBER_SPLIT_yesScp(item_obj)
    ...
    local yesScp = string.format('CHECK_CLIENT_USE_MULTIPLE_USE_STRING_GIVE_ITEM_NUMBER_SPLIT_BELONGING_CHECK("%s")',
        invItem:GetIESID());
...
function CHECK_CLIENT_USE_MULTIPLE_USE_STRING_GIVE_ITEM_NUMBER_SPLIT(item_id)
    ...
    if TryGetProp(itemObj, 'MaxStack', 0) == 1 or invItem.count == 1 then
        ...
        local titleText =
            ScpArgMsg("INPUT_CNT_D_D", "Auto_1", 1, "Auto_2", math.max(1, math.floor(invItem.count / div)))
        INPUT_NUMBER_BOX(nil, titleText, "RUN_CLIENT_USE_MULTIPLE_USE_STRING_GIVE_ITEM_NUMBER_SPLIT", 1, 1,
            invItem.count)
        multiple_string_give_item_numbersplit = tostring(item_id)
    end
end
...
function BEFORE_APPLIED_CHANGE_VIBORA(invItem)
    ...
    if itemClassName ~= 'SWD04_126_3' and itemClassName ~= 'DAG04_123_4' and itemClassName ~= 'SHD04_122_1' and
        itemClassName ~= 'DAG04_123_6' and itemClassName ~= 'SHD04_122_2' then
        return
    end
...
function CLIENT_CONVERT_CONVERT_MULTIPLE_TOKEN(item_obj)
    ...
    WARNINGMSGBOX_FRAME_OPEN(ScpArgMsg('ConvertMultipleTokenWarning{NAME}', 'NAME', TryGetProp(item, 'Name', 'None')),
        yesscp, 'None', nil, ScpArgMsg('WarningTeamBelonging'));
end
