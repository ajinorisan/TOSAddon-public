...
function monstercard_change_unequip()
    ...
    elseif g.slotindex <= 11 and g.slotindex >= 9 then
        ...
        g.slotindex = 0
        --[[for i, clsID in ipairs(g.cardlist) do
            print("g.cardlist[" .. i .. "]: " .. tostring(clsID))
        end
...
function monstercard_change_put_inv_to_warehouse()
    ...
    AUTO_CAST(text1)
    text1:SetText(
        "{s20}{ol}{#CCCC22}Cards are being brought in.{nl} Do not perform any other actions to prevent bugs.{nl}カード搬入中。バグ防止の為、{nl}その他の動作を行わないでください")
...
function monstercard_change_get_presetinfo()
    ...
    for index = 0, 11 do
        ...
        if g.presetcardcount[cardID] == nil then
            g.presetcardcount[cardID] = 3 -- 初めて出現した場合は1回目として初期化
            -- else
...
function monstercard_change_CARD_PRESET_APPLY_PRESET()
    ...
    elseif fromframe:IsVisible() == 1 and allnil == true then
        if page ~= nil then
            pc.ReqExecuteTx_NumArgs("SCR_TX_APPLY_CARD_PRESET", page)
            _DISABLE_CARD_PRESET_APPLY_SAVE_BTN()
            return
    ...
    elseif fromframe:IsVisible() == 1 and allnil == false then
