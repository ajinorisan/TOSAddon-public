...
function CC_HELPER_ON_INIT(addon, frame)
    ...
    if mapCls.MapType == "City" then
        ...
        local active_cid = session.loginInfo.GetAID();
        local FileLoc = string.format('../addons/%s/%s.json', addonNameLower, active_cid)
        g.cid = {}
        local cid, err = acutil.loadJSON(FileLoc, g.cid)
        if not cid then
