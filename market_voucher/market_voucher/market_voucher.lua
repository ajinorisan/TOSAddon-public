...
g.logpath = string.format('../addons/%s/log_2507.txt', addon_name_lower)
...
function g.mkdir_new_folder()
    ...
    g.settings_path = string.format("../addons/%s/%s/settings_2507.json", addon_name_lower, g.active_id)
...
function market_voucher_print(frame, ctrl, arg_str, arg_num)
    ...
    for i = 1, item_count do
        ...
        local line_text = ""
        if status == "sell" then
            status = g.ui_text(status)
            sumtotal_amount = sumtotal_amount + total_amount_val
            unit_price_val = unit_price_val / 0.9
            --[[line_text = string.format("%s%s ･ %s%s ･ %s%s ･ %s%s ･ %s%s ･ %s%s ･ %s",
                g.lang_trans("time of sale:"), date_str, g.lang_trans("name:"), name_str, g.lang_trans("item:"),
                item_str, g.lang_trans("quantity:"), quantity_str, g.lang_trans("unit price:"),
                GET_COMMAED_STRING(unit_price_val), g.lang_trans("total amount:"), GET_COMMAED_STRING(total_amount_val),
                status)]]
            line_text = string.format("%s%s ･ %s ･ %s ･ %s%s ･ %s%s ･ %s%s ･ %s",
                g.lang_trans("Sale Date/Time:"), date_str, name_str, item_str, g.lang_trans("quantity:"), quantity_str,
                g.lang_trans("unit price:"), GET_COMMAED_STRING(unit_price_val), g.lang_trans("total amount:"),
        ...
        elseif status == "buy" then
            ...
            --[[line_text = "{#DAA520}" ..
                    ...
                    g.lang_trans("time of sale:"), date_str, g.lang_trans("name:"), name_str, g.lang_trans("item:"),
                    item_str, g.lang_trans("quantity:"), quantity_str, g.lang_trans("unit price:"),
                    GET_COMMAED_STRING(unit_price_val), g.lang_trans("total amount:"),
                    GET_COMMAED_STRING(total_amount_val), status)]]
            line_text = "{#DAA520}" ..
                            string.format("%s%s ･ %s ･ %s ･ %s%s ･ %s%s ･ %s△%s ･ %s",
                    g.lang_trans("Purchase Date/Time:"), date_str, name_str, item_str, g.lang_trans("quantity:"),
                    quantity_str, g.lang_trans("unit price:"), GET_COMMAED_STRING(unit_price_val),
                    g.lang_trans("total amount:"), GET_COMMAED_STRING(total_amount_val), status)
        end
    ...
    local latest_date_str = string.match(g.settings[1], date_pattern)
    local earliest_date_str = string.match(g.settings[item_count], date_pattern)
    ...
    sum_total_amount_text:SetText(
        "{#FF0000}" .. g.ui_text("Total Sales:") .. GET_COMMAED_STRING(sumtotal_amount) .. "(" ..
            GET_COMMAED_STRING(rounded_number) .. "M)")
    sum_total_amount_text:ShowWindow(1)

    -- 期間のテキストを設定
    local period_text = frame:CreateOrGetControl("richtext", "date_text", 620, 740, 100, 30)
...
function MARKET_VOUCHER_ON_INIT(addon, frame)
    ...
    g.setup_hook_and_event(addon, "_CABINET_ITEM_BUY", "market_voucher__CABINET_ITEM_BUY", false)
...
function market_voucher__CABINET_ITEM_BUY(my_frame, my_msg)
    ...
    local item_obj = GetIES(cabinetItem:GetObject())
    local item_name = dictionary.ReplaceDicIDInCompStr(item_obj.Name)

    local sanitized_item_name = string.gsub(item_name, "-", "?")
    local reg_time = cabinetItem:GetRegSysTime()
    ...
    local quantity = tonumber(cabinetItem.sellItemAmount)

    local total_amount = tonumber(cabinetItem:GetCount())
