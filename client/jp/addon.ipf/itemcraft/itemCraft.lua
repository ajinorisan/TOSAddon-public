...
function CREATE_CRAFT_ARTICLE(frame)
	...
	while cls ~= nil do     
		if checkCraftFunc(cls, arg1, arg2) == true then
			local haveM = CRAFT_HAVE_MATERIAL(cls);	-- ??ñÏòâ Â™õ¬Ä?????
...
function CRAFT_UPDATE_PAGE(page, cls, haveMaterial, item)
	...
	if nil ~= skill and g_itemCraftFrameName ~= "itemcraft" then
		...
		if cls.IDSpc == 'Skill_Ability' and nil ~= abil then
			...
			-- ??æß???Î±ÄÍΩ??????Íæ©Ïî†??? ??àÍ∫º????ìÎ¶∞??? ??ÖÎ£ÑÊø???ïÎñé.
...
function SORT_INVITEM_BY_WORTH(a,b)
    ...
    if lifeTimeA ~= nil and lifeTimeB ~= nil then
        ...
        -- ?æÎãøÎ¶???Íæ©Ïî†??? ?Ï¢éÌÅπ??ItemLifeTime??0??Ä??ËπÇÎåÅ???ÅÏ®æ????
    ...
    if itemobj_a.GroupName == 'Gem' and itemobj_b.GroupName == 'Gem' then  -- ?Ï¢? ÁßªÎ?Î±??ÂØÉÏéå????ÔΩåÎº±????????ïÎú≤...
...
-- ??ñÏòâ??çÎ¶∞???∞‚ë∏?????Ï¶∫Áëú?Â™õ¬ÄÔßû¬Ä????àÎíóÔßû¬Ä Ôß£ÎåÑÍ≤??ïÎñé.
local function CHECK_MATERIAL_COUNT(recipecls, totalCount)    
    local havingItemCount = {}      -- Â™õ¬ÄÔßû¬Ä????àÎíó ??Ï¶???
    local requiredItemCount = {}    -- ??ñÏòâ???Íæ©ÏäÇ????Ï¶???
    ...
    for index = 1, 5 do
        ...
        if itemName ~= 'None' then
            ...
            if recipeItemCnt ~= nil and recipeItemCnt ~= 'None' then
                if requiredItemCount[itemName] == nil then  -- ÂØÉÏ?Îø??èÍΩå ??πÌäÇÊπ≤Í≥óÏø??Â™õÏàà? ?Íæ©Ïî†??Ôßè‚ë∏Ï§????âÏì£ ????áÎ¶∞ ?????map??∞Ï§à Â™õÏàà? ?´ÎÇÖÏ™üÁëú??æÎùï???
    ...
    local max_try_count = 210000000 -- Â™õ¬ÄÔß???Ï¶∫Êø°?Ôß§Ïíï?Êø?ÔßçÎöÆÎ±?????àÎíó Â™õÏíñ??
    ...
    for key, requiredCount in pairs(requiredItemCount) do        
        if requiredCount > havingItemCount[key] then
            return false, max_try_count    -- ??Ï¶??∫¬ÄË≠?
        end
    end

    return true, 0 -- Ë≠∞Í≥åÍµ?ÔßçÎöØ??
end
...
function CRAFT_DETAIL_CRAFT_EXEC_ON_SUCCESS(frame, msg, str, time)
    ...
    local validRecipeMaterial = GET_MATERIAL_VALIDATION_SCRIPT(recipecls);
    local IsValidRecipeMaterial = _G[validRecipeMaterial];    
    local map_classname = {}  -- ??ñÏòâ???Íæ©ÏäÇ???Íæ©Ïî†??Ôßè‚ë∏Ï§?
    local map_classID = {}
    local map_cnt = {}  -- ??ñÏòâ???Íæ©ÏäÇ???Íæ©Ïî†??ïÌÄ??Íæ©ÏäÇ Â™õÏíñ??
    local item_count = 0
    ...
    local extralist = {}    -- ‰ª•Î¨ê??Ôß£ÎåÑÍ≤????
    local ordered_list = {}
    local ordered_cnt = 1
    
    for i = 1, item_count do -- ??ñÏòâ???Íæ©ÏäÇ???Íæ©Ïî†??ñÏì£ ?Î™ÉÍπΩ?Î®?Ωå Â™õ¬Ä?Î™ÑÏÇ©??        
        ...
        if start == 1 then
            ...
            if invItem == nil then
				...
				ui.SetHoldUI(false);         
                return
            else    -- ??âÎñÜ???Íæ©Ïî†??ñÏî† Ë≠∞ÎåÅ???é„àÉ
            ...
            for j = 1, #candidate_list do
                ...
                if session.GetInvItemByGuid(candidate_list[j]:GetIESID()).isLockState == false then
                    if IsValidRecipeMaterial(classname, GetIES(candidate_list[j]:GetObject())) then                        
                        if geItemTable.IsStack(GetIES(candidate_list[j]:GetObject()).ClassID) == 1 then -- stack ???Íæ©Ïî†??ñÏî†??∞„àÉ
...
function CRAFT_MAKE_DETAIL_REQITEMS(ctrlset)	
	...
	for i = 1 , 5 do
		if recipecls["Item_"..i.."_1"] ~= "None" then
			local recipeItemCnt, invItemCnt, dragRecipeItem, invItem, recipeItemLv, invItemlist  = GET_RECIPE_MATERIAL_INFO(recipecls, i, GetMyPCObject());
			if invItemlist ~= nil then -- ??Ï¶??Íæ©Ïî†??ñÏî† ??æ©???∫ÏÇé??Ä????∞Ï§à ??§Îñé
				for j = 0, recipeItemCnt - 1 do
			...
			else -- ??Ï¶??Íæ©Ïî†??ñÏî† ??ΩÍπÆ?Î∫§Ïî†Ôß???∞Ï§à ??§Îñé
...
function SORT_PURE_INVITEMLIST(a,b)
	-- Â™õÏàà? ClassID??Â™õ¬ÄÔß???ñÏî™ ÂØÉÏéå???Î™É„Åà ??øÎíó ???????πÍΩ¶ ??éÎ£ÑÊø??Î∫£Ï†π??éÎíó ??•Îãî. 
	-- ?Î∫£Ï†π??ñÏêû : Ôßç„ÖºÏ≠??Ä???> ????ÂØÉÏ?ÎøïÁßª?> ??´‚îõ ???èÜ ??> ?Íæ©Ïò± Â™õÎ∫•????õÎãî > ??? ?????> ????àÍ∫º
	local itemobj_a = GetIES(a:GetObject());
	local itemobj_b = GetIES(b:GetObject());
...
function CRAFT_ITEM_ALL(itemSet, btn)    
    ...
    if recipeCls ~= nil then
        ...
        ignoreType = true; -- ËπÇÍæ®Î£????ΩÍ≤ï?±Ï?????•Îãî??Â´ÑÍ≥ó???ÂØÉÏéå??
    else        
	...
	if check_transcend == true or check_reinforce == true then  -- Â™õÎ∫•???•Îçâ?°Êø°??Î∫£Ï†π
	...
	if IS_EQUIP(itemObj) == true then		
		...
		-- end of ??ñÏòâ??Ï¶?Â™õÎ∫•???•Îçâ????èÌäÇ Ôß£ÎåÑÍ≤?
	...
	if (ignoreType or invItemadd.type == materialItemClassID) and invItemadd.count >= needcount then		
		...
		--?????å—âÏú≠??Ë´??Í≥†ÎìÉ Ë´õÏï∑Ïæ?ËπÇ¬ÄÂØ? 
		icon:SetColorTone('FFFFFFFF')
...
function CRAFT_PVP_MINE_ITEM_ALL(itemSet, btn)
	...
	elseif itemName == 'dummy_GabijaCertificate' then
		propName = 'GabijaCertificate' -- ?????ÔßùÏï∫Î™?Â™õ¬Ä??æ©Îπ? ?Í≥∏Ï†è
	elseif itemName == 'dummy_VakarineCertificate' then
		propName = 'VakarineCertificate' -- ?????ÔßùÏï∫Î™?Ë´õÎ∂øÎ≠ÖÁî±?ÍΩ? ?Í≥∏Ï†è
	elseif itemName == 'dummy_TeamBattleCoin' then
		propName = 'TeamBattleCoin' -- ?ÄÎ∞∞ÏΩî?
	elseif itemName == 'Event_2212_Snow_Key' then --?¥Î≤§??ÏΩîÏù∏?
...
function MAKE_DETAIL_REQITEMS(ctrlset)
	...
	if myActor:IsSit() == 1 then -- Ï©?? UIÏß∏Ïßï Ï®?? Ï®ãÏß†Ï®îÏß∞Ï©îÏßï(Ï®çÏ®ç??Ï®?Ï©çÏ≤∏? [?úÏ©î Ï©?? Ï®??Ïß∏ÏßïÏ®ås??? Ïß??f[: ?Ï©çÏ∫°???Ïß†Ï®çÏ®??Ï®???sÏß?140922.
		--make:ShowWindow(1)
		make:ShowWindow(0)
	else
		make:ShowWindow(0)
...
-- ?Íæ©Ïî†????ñÏòâ ?Î™ÑÎøâ??SetCraftState()????ÅÏäú???Ï¢? ??∞Î£û ??óÎ∏≥??çÌÄ???†Îº±??
-- ??Å„Åù?Î®?Ωå ??àÍ∂° msg ??ÅÏäú ?ø¬Ä?±—ãÎ∏Ø?Íæ®Ï§â ??
