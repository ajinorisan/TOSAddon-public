...
-- v1.0.7 お知らせをチャットのみに。コードリニューアル。プリセット機能移植
...
function ancient_autoset_frame_init()
    ...
    btn_aas:SetTextTooltip(g.lang == "Japanese" and
                               ...
                               "{ol}[AAS]{nl}Left-click:Register{nl}Right-click:Setting Release")

    if not g.settings.presetuse then
        g.settings.presetuse = 1
        g.save_settings()
    end

    local preset_check = topbg:CreateOrGetControl('checkbox', 'maney_check', 435, 285, 30, 30)
    AUTO_CAST(preset_check);
    preset_check:SetCheck(g.settings.presetuse)
    preset_check:SetEventScript(ui.LBUTTONUP, "ancient_autoset_preset_toggle")
    preset_check:SetTextTooltip(g.lang == "Japanese" and "{ol}[AAS]{nl}プリセット機能 使用/不使用 切替" or
                                    "{ol}[AAS]Preset function Use/Unuse switch")

end

function ancient_autoset_preset_toggle(frame, ctrl)

    local is_check = ctrl:IsChecked()

    if is_check == 0 then
        g.settings.presetuse = 0
    else
        g.settings.presetuse = 1
    end
    ui.SysMsg(g.lang == "Japanese" and "切替にはローディング必要です" or
                  "Loading is required for switching")
    g.save_settings()
end
...
function ANCIENT_AUTOSET_ON_INIT(addon, frame)
    ...
    if g.settings.presetuse == 1 then
