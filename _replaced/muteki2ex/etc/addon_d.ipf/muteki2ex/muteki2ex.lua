...
function MUTEKI2_INIT_CIRCLE(frame, buffObj)
    local image = frame:CreateOrGetControl("picture", "circle_" .. buffObj.ClassName, 0, 0, 50, 50)
...
function OVERLOAD_START(buffid, frame, buff)
    local gauge = frame
    local time = math.floor(buff.time / 1000)
    local buffSetting = MUTEKI2_GET_BUFF_SETTING(buffid)

    -- print(tostring(g.settings["buffList"][tostring(buffid)].color))
    -- gauge:SetColorTone('FFFF6666')
    gauge:SetColorTone(g.settings["buffList"][tostring(buffid)].color)
...
function OVERLOAD_COOLDOWN(buffid, frame)
    local gauge = frame
...
function MUTEKI2_ADD_CIRCLE_BUFF_TIMEUPDATE(buff, frame)
    ...
    time:RunUpdateScript("_MUTEKI2_ADD_CIRCLE_BUFF_TIMEUPDATE", 0.1)

end

function _MUTEKI2_ADD_CIRCLE_BUFF_TIMEUPDATE(time)
...
function MUTEKI2_UPDATE_POSITIONS()
    ...
    if #gaugeList > 0 then
        table.sort(gaugeList, function(a, b)
