...
function MUTEKI2_CREATE_SETTING_LIST(frame, gbox, index, buffid, buffSetting)
    ...
    local nico_chat = list:CreateOrGetControl('checkbox', 'nico_chat', 300, 90, 35, 35)
    AUTO_CAST(nico_chat)
    nico_chat:SetCheck(buffSetting.nico_chat)
    nico_chat:SetEventScript(ui.LBUTTONDOWN, 'MUTEKI2_CHANGE_EFFECT')
    nico_chat:SetEventScriptArgNumber(ui.LBUTTONDOWN, buffid)
    nico_chat:SetText(_translate('nico_chat'))
...
function MUTEKI2_CHANGE_EFFECT(list, control, isChecked, buffid)
    ...
    elseif control:GetName() == "nico_chat" then
        g.settings.buffList[tostring(buffid)].nico_chat = control:IsChecked()
    end
