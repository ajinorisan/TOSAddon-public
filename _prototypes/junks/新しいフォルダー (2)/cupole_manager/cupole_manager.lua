-- v1.0.1 クポル未登録の場合はsettingsから、個別登録済はpersonalから呼び出し。
...
function CUPOLE_MANAGER_ON_INIT(addon, frame)
    ...
    g.personalFileLoc = string.format('../addons/%s/%s.json', addonNameLower, g.cid)
    ...
    if mapCls.MapType == "City" then
        ...
        local file = io.open(g.personalFileLoc, "r")
        if file then
            local content = file:read("*all")
            file:close()
            g.personal = json.decode(content)
        else
...
function cupole_manager_CLOSE_CUPOLE_ITEM(frame, msg)
    ...
    local file = io.open(g.personalFileLoc, "w")
...
function cupole_manager_SET_CUPOLE_SLOTS(frame)
    ...
    if g.personal then
        equip_cupole_list = {g.personal["1"].id, g.personal["2"].id, g.personal["3"].id}
