...
local CommandParamList = {
    reset = {
        jp = "設定リセット",
        en = "Reset the all settings."
    },
    update = {
        jp = "表示を更新",
        en = "The additional information displayed will be updated."
    },
    restart = {
        jp = "現在の翻訳データを破棄して再始動",
        en = "Discard current translation data and restart."
    }
};
...
local Toukibi = {
    CommonResText = {
        jp = {
            ...
            Help = {
                ...
                Description = string.format(
                    "{#92D2A0}%sは次のパラメータで設定を呼び出してください。{/}", addonName),
                ParamDummy = "[パラメータ]",
                OrText = "または",
        ...
        en = {
            ...
            Help = {
                ...
                Description = string.format(
                    "{#92D2A0}To change settings of '%s', please call the following command.{/}", addonName),
    ...
    GetResData = function(self, TargetRes, Lang, Key)
        ...
        local CurrentRes = self:GetValue(TargetRes[Lang], Key) or self:GetValue(TargetRes["en"], Key) or
                               self:GetValue(TargetRes["jp"], Key);
        return CurrentRes;
    end,
    ...
    AddLog = function(self, Message, Mode, DisplayAddonName, OnlyDebugMode)
        ...
        Mode = Mode or "Info";
        if (not DebugMode) and Mode == "Info" then
            return
        end
        if (not DebugMode) and OnlyDebugMode then
            return
        end
        local HeaderText = "";
            ...
            if info.IsPC(handle) == 1 then
                ...
                if shopFrame ~= nil then
                    ...
                    if Me.ShopHandleList[FrameName] == nil then
                        ...
                        if sellType == AUTO_SELL_BUFF or sellType == AUTO_SELL_GEM_ROASTING or sellType ==
                            AUTO_SELL_SQUIRE_BUFF or sellType == AUTO_SELL_ENCHANTERARMOR or sellType ==
                            AUTO_SELL_APPRAISE or sellType == AUTO_SELL_PORTAL then
    ...
    while #Me.SwitchShopFrameList >= 1 do
        ...
        if objShopFrame ~= nil then
            if Me.ShopHandleList[strHandle] ~= nil then
                ...
                if sellType == AUTO_SELL_BUFF or sellType == AUTO_SELL_GEM_ROASTING or sellType == AUTO_SELL_SQUIRE_BUFF or
                    sellType == AUTO_SELL_ENCHANTERARMOR or sellType == AUTO_SELL_APPRAISE or sellType ==
                    AUTO_SELL_PORTAL then
...
function Me.UpdateData()
    ...
    elseif FunctionExists(keyboard.IsKeyPressed) then
        -- 2018/06/27パッチ後
        isPressed = keyboard.IsKeyPressed("RCTRL") == 1;
    else
        -- どっちの関数もなかった場合
        isPressed = false;
    end
...
function Me.RestartAddon()
    ...
    Me.ShopHandleList = {};
    Toukibi:AddLog(
        "現在保持していた翻訳データを放棄しました。(ファイルに保存したデータは破棄していません)",
        "Caution", true, false);
    -- 通常処理に戻る 
    Me.UpdateData();
end
