...
local settingsFileLoc = string.format('../addons/%s/settings2501.json', string.lower(addonName))
-- local RList = { Moring = 522, Witch = 620, Giltine = 628, Vasilisa = 655,Delmore=665,Jellyzele=670};
local acutil = require('acutil');

function g.mkdir_new_folder()
    local folder_path = string.format("../addons/%s", string.lower(addonName))
    local file_path = string.format("../addons/%s/mkdir.txt", string.lower(addonName))
...
function TOOLBOX_LOAD()
    local settings, err = acutil.loadJSON(settingsFileLoc)
...
function TOOLBOX_OPENFRAME()
    ...
    AUTO_CAST(close)
    local margin = close:GetMargin()
    close:SetMargin(0, 2, 51, 0)
    close:Resize(24, 24)

    local minimize = GET_CHILD(frame, "minimize")
    AUTO_CAST(minimize)
    minimize:SetMargin(135, 2, 2, 0)
    minimize:Resize(24, 24)
    ...
    silver_content:SetText("{ol}" .. GET_COMMAED_STRING(GET_TOTAL_MONEY_STR()));
    i = i + 1;
    -- 傭兵
    ...
    PVP1_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "MISC_PVP_MINE2", "0")));
    i = i + 1;
    -- 加比婭
    ...
    Gabi1_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "GabijaCertificate", "0")));
    i = i + 1;
    -- 瓦卡麗涅
    ...
    Vaka1_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "VakarineCertificate", "0")));
    i = i + 1;
    -- 拉達
    ...
    Rada1_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "RadaCertificate", "0")));
    i = i + 1;
    ...
    Jurate1_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "JurateCertificate", "0")));
    i = i + 1;
    -- TOS鑄幣
    ...
    TOS_title:SetEventScript(ui.LBUTTONUP, "TOOLBOX_REQ_TOS_SHOP_OPEN");
    local TOS_content = TOS_group:CreateOrGetControl('richtext', 'TOS_content', 40, 6, 41, 30);
    TOS_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "EVENT_TOS_WHOLE_TOTAL_COIN", "0")));
    i = i + 1;
    -- 綜合
    ...
    Con_title:SetEventScript(ui.LBUTTONUP, "TOOLBOX_COLLECTION");
    local Con_content = Con_group:CreateOrGetControl('richtext', 'Con_content', 40, 6, 41, 30);
    Con_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "CONTENTS_TOTAL_POINT", "0")));
    i = i + 1;
    -- 王國
    ...
    RE_title:SetEventScript(ui.LBUTTONUP, "TOOLBOX_REPUTATION");
    local RE_content = RE_group:CreateOrGetControl('richtext', 'RE_content', 40, 6, 41, 30);
    RE_content:SetText("{ol}" .. GET_COMMAED_STRING(TryGetProp(aObj, "REPUTATION_COIN_EP13", "0")));
    i = i + 1;
    -- TP
    ...
    TP_content:SetText("{ol}" .. GET_COMMAED_STRING(aObj.GiftMedal + aObj.PremiumMedal));
    i = i + 1;
    -- 活動TP
    ...
    FTP_content:SetText("{ol}" .. GET_COMMAED_STRING(aObj.Medal));
    i = i + 1;
    -- 重量(%)
    ...
    WS_content:SetText("{ol}" .. GET_COMMAED_STRING(GET_PLAYER_GEAR_SCORE(pc)));
    i = i + 1;
    -- 米酒
    ...
    WSSSS_title:SetGravity(ui.RIGHT, ui.TOP)
end
