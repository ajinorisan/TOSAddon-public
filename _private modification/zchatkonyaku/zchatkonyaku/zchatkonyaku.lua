...
function ZCHATKONYAKU_ON_INIT(addon, frame)
    ...
    frame:RunUpdateScript("KONYAKU_TRANSLATE_RECV", 0.5)
    -- タイマー準備
    ...
    for i = 1, #g.MSG_PATTERN do
        ...
        print(tostring(type_pat));
    end
]]
    -- g.SetupHook(ZCHATKONYAKU_CHAT_SYSTEM, "CHAT_SYSTEM")
    -- print(tostring(g.MSG_PATTERN))
    -- CHAT_SYSTEM("発言フックOK");

end
--[[
function ZCHATKONYAKU_CHAT_SYSTEM(msg, color)
    ...
    while cnt < 10 do
        ...
        if len <= 13 then
            ...
            if #msga > 1 then
                ...
                -- ZCHATKONYAKU_CHAT_SYSTEM(msg)
                -- print("cmd_nm " .. cmd_nm);
                -- print("msg " .. msg);
                ui.SysMsg("{@st64}" .. g.msgtype .. ":" .. cmd_nm .. ":" .. msg)
                NICO_CHAT(g.msgtype .. ":" .. cmd_nm .. ":" .. msg)
                -- システムメッセージでは名前は省略
