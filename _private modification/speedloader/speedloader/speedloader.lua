...
function SPEEDLOADER_ON_FRAME_INIT(frame)
    ...
    infoText:SetText("{@st42}Reload{/}")
    infoText:EnableHitTest(1)
    infoText:SetEventScript(ui.LBUTTONUP, "SPEEDLOADER_EFFECT_SETTING")
...
function SPEEDLOADER_EFFECT_SETTING(frame, ctrl)
        ...
        if (buff.over == 10) then
            if (IsBattleState(GetMyPCObject()) == 1) then
                ...
                if (SpeedLoader.Settings.ShowVisual == 1) then
                    ...
                    effect.AddActorEffectByOffset(actor, "F_pattern025_loop", 2.5, "MID", true, true);
            ...
            -- effect.AddActorEffectByOffset(actor, "F_pattern025_loop", 1, "MID", true, true);
            -- effect.AddActorEffectByOffset(actor, effectName, 0.15, "MID", true, true);
            -- end
            ...
            gauge:SetColorTone("FFCCCC22");
        end
    elseif (buff == nil) then
        effect.DetachActorEffect(actor, "F_pattern025_loop", 0.1);
        effect.DetachActorEffect(actor, effectName, 0.1);
        gauge:SetPoint(0, 10);
        gauge:SetSkinName("speedloader_gauge_yellow");
        gauge:SetColorTone("FFCCCC22");
    end
